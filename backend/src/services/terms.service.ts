...
import EntityAlias = Levelup.V2.Cm.Entity.Term;
import ApiAlias = Levelup.V2.Cm.Api.Terms;
import DocumentAlias = Levelup.V2.Cm.Model.TermDocument;
import EventPayloadsAlias = Levelup.V2.Events.Payloads.Cm.Term;
...
export default class TermsService extends BaseService {
  protected ENTITY = 'term' as const;
  ...
  public async _generateSnapshotsObject(
    ...
    try {
      ...
      const taxonomy = await this.taxonomyModel.findById(
        getItemIdsFromOldAndNewData(new_data.taxonomy, old_data?.taxonomy)
      );

      this.logger.debug(this._generateSnapshotsObject.name, new_data.taxonomy, old_data?.taxonomy, taxonomy?.name);

      if (taxonomy) {
        result.taxonomy = extractPartialObject(taxonomy.toObject(), [
          '_id',
          'slug',
          'name',
          'is_hierarchical',
  ...
  _applyFilters({
    ...
    // -- taxonomy
    filter = createStringFilter<DocumentProperties>(q, totalQ, filters.taxonomy, 'taxonomy');
    q = filter.q;
    totalQ = filter.totalQ;

    // -- name
    if (TermSchemaFields['name']) {
  ...
  public async list(
    ...
    try {
      ...
      let q: mongoose.QueryWithFuzzySearch<EntityAlias> = this.termModel.find();
      let totalQ: mongoose.QueryWithFuzzySearch<EntityAlias> = this.termModel.where();
  ...
  public async create(
    ...
    try {
       ...
       */
      data = await TermSanitizers.sanitizeCreateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = TermValidators.validateCreateBody(data);
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.created>(events.cm.term.created, { data: doc });
  ...
  public async update(
    ...
    try {
       ...
       * await sanitize data here
       */
      data = await TermSanitizers.sanitizeUpdateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = TermValidators.validateUpdateBody(data);
      if (error) throw error;
       ...
       * Create search meta
       */
      if (TermSchemaFields['search_meta']) {
       ...
       */
      const doc = await this.termModel.findByIdAndUpdate(
        id,
        {
          ...docObject,
          $addToSet: {
       ...
       */
      this.eventDispatcher.dispatch<EventPayloadsAlias.updated>(events.cm.term.updated, { data: doc });
  ...
  public async delete(id: string, authData: Levelup.V2.Security.AuthData): Promise<ApiAlias.Delete.Response> {
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.term.deleted, { data: doc });
  ...
  public async restore(id: string, authData: Levelup.V2.Security.AuthData): Promise<ApiAlias.Delete.Response> {
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.term.restored, { data: doc });
