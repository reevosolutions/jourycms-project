...
import EntityAlias = Levelup.V2.Cm.Entity.Taxonomy;
import ApiAlias = Levelup.V2.Cm.Api.Taxonomies;
import DocumentAlias = Levelup.V2.Cm.Model.TaxonomyDocument;
import EventPayloadsAlias = Levelup.V2.Events.Payloads.Cm.Taxonomy;
 ...
 */
@Service()
export default class TaxonomiesService extends BaseService {

  protected readonly ENTITY = 'taxonomy' as const;
  ...
  */
  public async _generateSnapshotsObject(
    ...
    try {
      ...
      const result: Levelup.V2.Utils.Complete<EntityAlias['snapshots']> = {
  ...
  _applyFilters({ query, q, totalQ, opt, authData }: {
    ...
    if (authData?.attributed?.company?._id) filters.company = authData?.attributed.company._id;
    else if (authData?.current?.company?._id) filters.company = authData?.current.company._id;
    if (authData?.current?.app) (filters).app = authData?.current.app._id;

    // -- attributed:app
    if (TaxonomySchemaFields['app']) {
    ...
    // -- created_by
    if (TaxonomySchemaFields['created_by']) {
  ...
  public async list(
    ...
    try {
      ...
      let q: mongoose.QueryWithFuzzySearch<EntityAlias> = this.taxonomyModel.find();
      let totalQ: mongoose.QueryWithFuzzySearch<EntityAlias> = this.taxonomyModel.where();
  ...
  */
  public async getById(
    ...
    try {
      ...
      const q = this.taxonomyModel.findById(id);
      /**
  ...
  */
  public async getByName(
    ...
    try {
      ...
      const q = this.taxonomyModel.findOne({ name });
      /**
       * @description Lean the query else if needed to not do so
       */
      if (!opt.dont_lean) q.lean();
  ...
  public async create(
    ...
    try {
       ...
       */
      data = await TaxonomySanitizers.sanitizeCreateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = TaxonomyValidators.validateCreateBody(data);
       ...
       * Create tracking ID
       */
      if (TaxonomySchemaFields['tracking_id'] && Object.keys(ITEM_SHORTCUTS).includes(this.ENTITY)) {
        docObject['tracking_id'] = await createTrackingId(this.ENTITY, this.taxonomyModel);
      }

      /**
       * Create search meta
       */
      if (TaxonomySchemaFields['search_meta']) {
       ...
       */
      const doc = await this.taxonomyModel.create(docObject);

      if (!doc) throw new exceptions.InternalServerError('Failed to create the object');

      this.eventDispatcher.dispatch<EventPayloadsAlias.created>(events.cm.taxonomy.created, { data: doc });
  ...
  */
  public async update(
    ...
    try {
       ...
       * await sanitize data here
       */
      data = await TaxonomySanitizers.sanitizeUpdateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = TaxonomyValidators.validateUpdateBody(data);
      if (error) throw error;
       ...
       */
      const old = await this.taxonomyModel.findById(id);
       ...
       */
      const doc = await this.taxonomyModel.findByIdAndUpdate(
        id,
        {
          ...docObject,
          $addToSet: {
       ...
       */
      this.eventDispatcher.dispatch<EventPayloadsAlias.updated>(events.cm.taxonomy.updated, { data: doc });
  ...
  */
  public async delete(
    ...
    try {
      ...
      const doc = await this.taxonomyModel.findByIdAndUpdate(id, {
        is_deleted: true, 
        deleted_at: new Date(),
        $addToSet: {
          updates: updateObject
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.taxonomy.deleted, { data: doc });
  ...
  */
  public async restore(
    ...
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.taxonomy.restored, { data: doc });
