...
import EntityAlias = Levelup.V2.Cm.Entity.Comment;
import ApiAlias = Levelup.V2.Cm.Api.Comments;
import DocumentAlias = Levelup.V2.Cm.Model.CommentDocument;
import EventPayloadsAlias = Levelup.V2.Events.Payloads.Cm.Comment;
...
export default class CommentsService extends BaseService {
  ...
  protected ENTITY = 'comment' as const;
  ...
  */
  ...
  _applyFilters({ query, q, totalQ, opt, authData }: {
    ...
    // -- created_by
    if(CommentSchemaFields['created_by']){
  ...
  public async list(
    ...
    try {
      ...
      let q: mongoose.QueryWithFuzzySearch<EntityAlias> = this.commentModel.find();
      let totalQ: mongoose.QueryWithFuzzySearch<EntityAlias> = this.commentModel.where();
  ...
  */
  public async getById(
    ...
    try {
      ...
      const q = this.commentModel.findById(id);
      /**
  ...
  public async create(
    ...
    try {
       ...
       */
      data = await CommentSanitizers.sanitizeCreateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = CommentValidators.validateCreateBody(data);
       ...
       * Create tracking ID
       */
      if (CommentSchemaFields['tracking_id'] && Object.keys(ITEM_SHORTCUTS).includes(this.ENTITY)) {
        docObject['tracking_id'] = await createTrackingId(this.ENTITY, this.commentModel);
      }

      /**
       * Create search meta
       */
      if (CommentSchemaFields['search_meta']) {
       ...
       */
      const doc = await this.commentModel.create(docObject);

      if (!doc) throw new exceptions.InternalServerError('Failed to create the object');

      this.eventDispatcher.dispatch<EventPayloadsAlias.created>(events.cm.comment.created, { data: doc });
  ...
  */
  public async update(
    ...
    try {
       ...
       * await sanitize data here
       */
      data = await CommentSanitizers.sanitizeUpdateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = CommentValidators.validateUpdateBody(data);
      if (error) throw error;
       ...
       */
      const old = await this.commentModel.findById(id);
       ...
       */
      this.eventDispatcher.dispatch<EventPayloadsAlias.updated>(events.cm.comment.updated, { data: doc });
  ...
  */
  public async delete(
    ...
    try {
      ...
      const doc = await this.commentModel.findByIdAndUpdate(id, {
        is_deleted: true, 
        deleted_at: new Date(),
        $addToSet: {
          updates: updateObject
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.comment.deleted, { data: doc });
  ...
  */
  public async restore(
    ...
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.comment.restored, { data: doc });
