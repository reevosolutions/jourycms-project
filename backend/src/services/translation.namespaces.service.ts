...
import DocumentAlias = Levelup.V2.Cm.Translation.Model.NamespaceDocument;
import EventPayloadsAlias = Levelup.V2.Events.Payloads.Cm.Translation.Namespace;
 ...
 */
@Service()
export default class TranslationNamespacesService extends BaseService {

  protected readonly ENTITY = 'translationNamespace' as const;
  ...
  _applyFilters({ query, q, totalQ, opt, authData }: {
    ...
    // -- attributed:app
    if (TranslationNamespaceSchemaFields['app']) {
    ...
    // -- created_by
    if (TranslationNamespaceSchemaFields['created_by']) {
  ...
  public async list(
    ...
    try {
      ...
      let q: mongoose.QueryWithFuzzySearch<EntityAlias> = this.translationNamespaceModel.find();
      let totalQ: mongoose.QueryWithFuzzySearch<EntityAlias> = this.translationNamespaceModel.where();
  ...
  */
  public async getById(
    ...
    try {
      ...
      const q = this.translationNamespaceModel.findById(id);
      /**
  ...
  */
  public async getByName(
    ...
    try {
      ...
      const q = this.translationNamespaceModel.findOne({ name });
      /**
       * @description Lean the query else if needed to not do so
       */
      if (!opt.dont_lean) q.lean();
  ...
  public async create(
    ...
    try {
       ...
       */
      data = await TranslationNamespaceSanitizers.sanitizeCreateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = TranslationNamespaceValidators.validateCreateBody(data);
       ...
       * Create tracking ID
       */
      if (TranslationNamespaceSchemaFields['tracking_id'] && Object.keys(ITEM_SHORTCUTS).includes(this.ENTITY)) {
        docObject['tracking_id'] = await createTrackingId(this.ENTITY, this.translationNamespaceModel);
      }

      /**
       * Create search meta
       */
      if (TranslationNamespaceSchemaFields['search_meta']) {
       ...
       */
      const doc = await this.translationNamespaceModel.create(docObject);

      if (!doc) throw new exceptions.InternalServerError('Failed to create the object');

      this.eventDispatcher.dispatch<EventPayloadsAlias.created>(events.cm.translation.namespace.created, { data: doc });
  ...
  */
  public async update(
    ...
    try {
       ...
       * await sanitize data here
       */
      data = await TranslationNamespaceSanitizers.sanitizeUpdateBody(data, authData);

      /**
       * Validate data here
       */
      const { error } = TranslationNamespaceValidators.validateUpdateBody(data);
      if (error) throw error;
       ...
       */
      const old = await this.translationNamespaceModel.findById(id);
       ...
       */
      const doc = await this.translationNamespaceModel.findByIdAndUpdate(
        id,
        {
          ...docObject,
          $addToSet: {
       ...
       */
      this.eventDispatcher.dispatch<EventPayloadsAlias.updated>(events.cm.translation.namespace.updated, { data: doc });
  ...
  */
  public async delete(
    ...
    try {
      ...
      const doc = await this.translationNamespaceModel.findByIdAndUpdate(id, {
        is_deleted: true, 
        deleted_at: new Date(),
        $addToSet: {
          updates: updateObject
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.translation.namespace.deleted, { data: doc });
  ...
  */
  public async restore(
    ...
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.translation.namespace.restored, { data: doc });
...
export const levelupExportServicePermissions: Levelup.V2.SystemStructure.ExportServicePermissions<TLevelupExportServicePermissionsGroups> = () => ({
  GROUPS: {
    "translation namespaces": "",
  },
  PERMISSIONS: {
    "translation namespaces": [
      'create new translation namespace',
      'update own translation namespace',
      'update any translation namespace',
      'delete own translation namespace',
      'delete any translation namespace',
      'view own translation namespaces',
