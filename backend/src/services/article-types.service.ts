...
import EntityAlias = Levelup.V2.Cm.Entity.ArticleType;
import ApiAlias = Levelup.V2.Cm.Api.ArticleTypes;
import DocumentAlias = Levelup.V2.Cm.Model.ArticleTypeDocument;
import EventPayloadsAlias = Levelup.V2.Events.Payloads.Cm.ArticleType;
  ...
  */
  ...
  public async create(
    ...
    try {
      ...
      data.company = authData?.current?.company?._id ? authData?.current?.company?._id : opt?.bypass_authorization || (authData.current?.service?.name && !authData.current?.service?.is_external) ? data.company : undefined;
      ...
      if (authData?.current?.company?._id && authData?.current?.company?._id !== data.company && !authData?.current?.company?.attributes?.companies.includes(data.company)) throw new exceptions.UnauthorizedException('You are not allowed to create this object for this company');
       ...
       * Create tracking ID
       */
      if (ArticleTypeSchemaFields['tracking_id'] && Object.keys(ITEM_SHORTCUTS).includes('ArticleType')) {
        const entity = 'ArticleType' as const;
        docObject['tracking_id'] = await createTrackingId(this.ENTITY, this.articleTypeModel);
      }
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.created>(events.cm.articleType.created, { data: doc });
  ...
  */
  public async update(
    ...
    try {
       ...
       */
      this.eventDispatcher.dispatch<EventPayloadsAlias.updated>(events.cm.articleType.updated, { data: doc });
  ...
  */
  public async delete(
    ...
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.articleType.deleted, { data: doc });
  ...
  */
  public async restore(
    ...
    try {
      ...
      this.eventDispatcher.dispatch<EventPayloadsAlias.deleted>(events.cm.articleType.restored, { data: doc });
...
export const levelupExportServicePermissions: Levelup.V2.SystemStructure.ExportServicePermissions<TLevelupExportServicePermissionsGroups> = () => ({
  ...
  PERMISSIONS: {
    "article types": [
      ...
      'update own article type',
      'update any article type',
      'delete own article type',
      'delete any article type',
      'view own article types',
