 ...
 * @returns string
 */
export const generateApiKeySignature = async (api_key: string, secret_key: string) => {
  ...
  if (typeof window !== "undefined") {
    ...
    if (oldSignature) {
      const timestamp = oldSignature?.substring(18, 28);
  ...
  if (typeof window !== "undefined") window.localStorage.setItem('sign', JSON.stringify(signature));
  return signature;
}
 ...
 * @returns boolean
 */
export const verifyApiKeySignature = async (signature: string) => {
  let valid = false;
  try {
    const api_key = signature?.substring(89);
    if (!api_key) throw new Error('INVALID_SIGNATURE: ');
    // TODO: get secret_key from database
    const secret_key = process.env.NEXT_PUBLIC_API_SECRET_KEY || '';
    const salt = signature?.substring(1, 17);
    const timestamp = signature?.substring(18, 28);
    const timestampSalt = (await md5(`${secret_key}${timestamp}`)).substring(0, 16);
    const hash = signature?.substring(28, 88);
    if (timestampSalt !== salt) throw new Error('INVALID_SIGNATURE: ' + timestampSalt + ' - ' + salt);

    // check if signature is valid by timestamp
    const now = moment().unix();
    const diff = now - parseInt(timestamp);
    if (diff > 3600) throw new Error('INVALID_SIGNATURE.expired: ' + diff);
...
export async function verifyHashedString(str: string, hash: string) {
