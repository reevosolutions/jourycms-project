 ...
 * @returns boolean
 */
export function userHasRoleGroupOrAbove(
  currentUserRoleGroup: Levelup.V2.Auth.Entity.TRoleGroup | undefined,
  roleGroup: Levelup.V2.Auth.Entity.TRoleGroup
) {
  if (!currentUserRoleGroup || !roleGroup) return false;
  return ROLE_GROUP_ORDER[currentUserRoleGroup] <= ROLE_GROUP_ORDER[roleGroup];
}

export function userMustBeLinkedToCompany(
  currentUserRoleGroup: Levelup.V2.Auth.Entity.TRoleGroup | undefined
) {
  if (!currentUserRoleGroup) return false;
  return (
    currentUserRoleGroup !== "users" &&
    !userHasRoleGroupOrAbove(currentUserRoleGroup, "application_administrators")
  );
}

export const roleGroupList = (groups: Levelup.V2.Auth.Entity.TRoleGroup[]) =>
  groups;
