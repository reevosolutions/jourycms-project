...
/**
 * @description Cache manager for Products
 */
export default class ProductsCacheManager
  implements Levelup.V2.CacheManager.EntityCacheManager<"product">
{
  private logger: LoggerService;

  private static instance: ProductsCacheManager;
  private readonly cache: CacheManager;

  private readonly EXPIRATION = 3600 * 48;
  private readonly ENTITY = "product" as const;
  ...
  public static getInstance(): ProductsCacheManager {
    if (!ProductsCacheManager.instance) {
      ProductsCacheManager.instance = new ProductsCacheManager();
    }
    return ProductsCacheManager.instance;
  }
  ...
  public async list(
    config: {
      company?: string | null;
      query?: Levelup.V2.CacheManager.TListQueryParams<"product">;
      force_load_from_db?: boolean;
      filter?: (
        item: Levelup.V2.SystemStructure.EntityType<"product">
      ) => boolean;
    } = {
      query: {} as any,
      force_load_from_db: true,
    }
  ): Promise<Levelup.V2.SystemStructure.EntityType<"product">[]> {
    return this.cache.list(this.ENTITY, config);
  }

  public async set(
    id: string,
    value: Partial<Levelup.V2.SystemStructure.EntityType<"product">>,
    company?: string | null
  ) {
