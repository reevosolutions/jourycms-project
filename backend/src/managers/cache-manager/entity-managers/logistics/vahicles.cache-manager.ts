...
/**
 * @description Cache manager for Vehicles
 */
export default class VehiclesCacheManager
  implements Levelup.V2.CacheManager.EntityCacheManager<"vehicle">
{
  private logger: LoggerService;

  private static instance: VehiclesCacheManager;
  private readonly cache: CacheManager;

  private readonly EXPIRATION = 3600 * 48;
  private readonly ENTITY = "vehicle" as const;
  ...
  public static getInstance(): VehiclesCacheManager {
    if (!VehiclesCacheManager.instance) {
      VehiclesCacheManager.instance = new VehiclesCacheManager();
    }
    return VehiclesCacheManager.instance;
  }
  ...
  public async list(
    config: {
      company?: string | null;
      query?: Levelup.V2.CacheManager.TListQueryParams<"vehicle">;
      force_load_from_db?: boolean;
      filter?: (
        item: Levelup.V2.SystemStructure.EntityType<"vehicle">
      ) => boolean;
    } = {
      query: {} as any,
      force_load_from_db: true,
    }
  ): Promise<Levelup.V2.SystemStructure.EntityType<"vehicle">[]> {
    return this.cache.list(this.ENTITY, config);
  }

  public async set(
    id: string,
    value: Partial<Levelup.V2.SystemStructure.EntityType<"vehicle">>,
    company?: string | null
  ) {
