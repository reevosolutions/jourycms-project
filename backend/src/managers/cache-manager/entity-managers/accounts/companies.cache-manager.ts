...
/**
 * @description Cache manager for Companies
 */
export default class CompaniesCacheManager
  implements Levelup.V2.CacheManager.EntityCacheManager<"company">
{
  private logger: LoggerService;

  private static instance: CompaniesCacheManager;
  private readonly cache: CacheManager;

  private readonly EXPIRATION = 3600 * 48;
  private readonly ENTITY = "company" as const;
  ...
  public static getInstance(): CompaniesCacheManager {
    if (!CompaniesCacheManager.instance) {
      CompaniesCacheManager.instance = new CompaniesCacheManager();
    }
    return CompaniesCacheManager.instance;
  }
  ...
  public async list(
    config: {
      company?: string | null;
      query?: Levelup.V2.CacheManager.TListQueryParams<"company">;
      force_load_from_db?: boolean;
      filter?: (
        item: Levelup.V2.SystemStructure.EntityType<"company">
      ) => boolean;
    } = {
      query: {} as any,
      force_load_from_db: true,
    }
  ): Promise<Levelup.V2.SystemStructure.EntityType<"company">[]> {
    return this.cache.list(this.ENTITY, config);
  }

  public async set(
    id: string,
    value: Partial<Levelup.V2.SystemStructure.EntityType<"company">>,
    company?: string | null
  ) {
