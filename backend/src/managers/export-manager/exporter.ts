...
import { transformObjectToString } from "../../utilities/objects";
import exportFormEntries from "./export-form-entries";

export { exportFormEntries };
...
export default class Exporter extends BaseService {
  ...
  public addData<T extends string, D = any>(
    columns:
      | {
          [K in T]: Partial<excelJS.Column> & {
            key?: K;
          };
        }
      | (() => {
    ...
    data: D[],
    mapper: (item: D) => { [Key in T]: unknown }
  ): void {
    this._mapper = mapper;
    this._data = data.map((el) => mapper(el));
    this._sheets[this._last_sheet].columns = Object.values(typeof columns === "function" ? columns() : columns);
  ...
  public setLabels(
    sheetName: string,
    labels: {
      [f: string]: { label: string; value: any };
    }
  ): void {
