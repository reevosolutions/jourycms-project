...
export const loadServicePermissions = async () => {
    try {
        ...
        if (!(apps === null || apps === void 0 ? void 0 : apps.length))
            throw new exceptions.LevelupException('There is no apps created.');
        /**
                    ...
                    for (let idx = 0; idx < apps.length; idx++) {
                        ...
                        if (module.levelupExportServicePermissions) {
                            ...
                            for (let idx = 0; idx < Object.keys(GROUPS).length; idx++) {
                                ...
                                try {
                                    const { data: { data, error } } = await levelupHttpClient.post(`${config.http.services.api.auth}/permission-groups`, {
                                        data: {
                                ...
                                for (let index = 0; index < perms.length; index++) {
                                    ...
                                    try {
                                        const { data: { data, error } } = await levelupHttpClient.post(`${config.http.services.api.auth}/permissions`, {
                                            data: {
                                                app: app === null || app === void 0 ? void 0 : app._id,
                                                group,
                                                name,
