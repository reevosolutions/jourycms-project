...
import CustomPagination from "@/themes/miqat/components/pagination";
import PostCard from "@/themes/miqat/components/post-card";
...
> = ({
  ...
  count: _count = 12,
    ...
    const { scrollMainView } = useScroll();
    /* -------------------------------------------------------------------------- */
    /*                                    STATE                                   */
    /* -------------------------------------------------------------------------- */
          ...
          <div className="">
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 lg:gap-10 xl:grid-cols-4">
              {filteredItems.map((item, index) => (
                <PostCard key={index} data={item} edge={data?.edge} articleTypeSlug={articleTypeSlug} />
              ))}
            </div>
            {showPagination && (data?.pagination?.pages || 0) > 1 && (
              <div className="my-8 mt-12">
                <CustomPagination totalCount={data?.pagination?.total} currentPage={page} pageSize={count} onPageChange={setPage} className=" text-lg" />
              </div>
            )}
          </div>
        )}
