...
const AdminSidebar: React.FC<Props> = () => {
    ...
    for (const routeKey of Object.keys(adminRoutes)) {
      if (
        ...
        if (slotResult.slot === "articleTypes") {
          ...
          const result = await slotResult.result(types);
          _routes = { ..._routes, ...result };
        }
      } else {
        _routes = {
          ..._routes,
  ...
  return (
    <Sidebar>
      <SidebarHeader>
        <Link href={"/"}
        className=" hocus:text-primary-600 transition-all p-2 flex items-center gap-3"
        
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 444.2 444.2" fill="currentColor">
            <path
          ...
          <span className="  font-bold text-xl">{"الرئيسية"}</span>
        </Link>
      </SidebarHeader>
