  ...
  return (
    <ul className={clsx("pagination-container", "my-8 flex justify-center items-center space-x-1", className)}>
      ...
      >
        <div
          className={clsx("pagination-item min-w-[28px] rounded-full p-2 text-center ", {
            "selected bg-white text-primary-700": currentPage === 1,
            " cursor-pointer bg-white/40 text-primary-500 transition-colors duration-500 hover:bg-primary-50 hover:text-primary-700   ":
              currentPage !== 1,
          })}
        >
          <Icons.Chevron.Left className="h-5 w-5 text-primary-500  rtl:rotate-180" />
        </div>
      </li>
      {paginationRange.map((pageNumber, index) => {
        if (pageNumber === DOTS) {
          return (
            <li key={`${pageNumber}${index}`} className="pagination-item dots px-2 text-primary-400">
              &#8230;
            </li>
          );
        }
        ...
        return (
          <li
            ...
            className={clsx("pagination-item min-w-[36px] rounded-full px-2 py-1 text-center ", {
              "selected bg-primary-50/50 text-primary-600": pageNumber === currentPage,
              " cursor-pointer bg-white/40 text-primary-500 transition-colors duration-500 hover:bg-primary-50 hover:text-primary-700   ":
      ...
      >
        <div
          className={clsx("pagination-item min-w-[28px] rounded-full p-2 text-center ", {
            "selected bg-white text-primary-700": currentPage === lastPage,
            " cursor-pointer bg-white/40 text-primary-500 transition-colors duration-500 hover:bg-primary-50 hover:text-primary-700   ":
              currentPage !== lastPage,
          })}
        >
          <Icons.Chevron.Right className="h-5 w-5 text-primary-500 rtl:rotate-180" />
        </div>
      </li>
      <li>
        <div className="rounded-full bg-white/40 px-2 hover:bg-primary-50 hover:text-primary-700  ">
          <select
            onChange={(event) => onPage(Number.parseInt(event.target.value))}
            className="cursor-pointer bg-transparent py-1  text-primary-500 transition-colors duration-500 hover:text-primary-700   "
            value={currentPage}
          >
