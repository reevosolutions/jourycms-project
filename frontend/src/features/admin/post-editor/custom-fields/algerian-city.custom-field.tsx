import React from "react";
import SelectCustomField from "./select.custom-field";
import initLogger, { LoggerContext } from "@/lib/logging";

const logger = initLogger(LoggerContext.COMPONENT, "algerian-city.custom-field");

export const cities = [
  {
    code: "01001",
    name: "تيمقتن",
    state_code: "01",
  },
  {
    code: "01002",
    name: "بودة",
    state_code: "01",
  },
  {
    code: "01003",
    name: "أولاد أحمد تيمي",
    state_code: "01",
  },
  {
    code: "01004",
    name: "أدرار",
    state_code: "01",
  },
  {
    code: "01005",
    name: "فنوغيل",
    state_code: "01",
  },
  {
    code: "01006",
    name: "إن زغمير",
    state_code: "01",
  },
  {
    code: "01007",
    name: "رقان",
    state_code: "01",
  },
  {
    code: "01008",
    name: "سالي",
    state_code: "01",
  },
  {
    code: "01009",
    name: "السبع",
    state_code: "01",
  },
  {
    code: "01010",
    name: "تسابيت",
    state_code: "01",
  },
  {
    code: "01011",
    name: "تامست",
    state_code: "01",
  },
  {
    code: "01012",
    name: "تامنطيط",
    state_code: "01",
  },
  {
    code: "01013",
    name: "تيت",
    state_code: "01",
  },
  {
    code: "01014",
    name: "زاوية كنتة",
    state_code: "01",
  },
  {
    code: "01015",
    name: "اقبلي",
    state_code: "01",
  },
  {
    code: "01016",
    name: "أولف",
    state_code: "01",
  },
  {
    code: "02001",
    name: "تلعصة",
    state_code: "02",
  },
  {
    code: "02002",
    name: "الزبوجة",
    state_code: "02",
  },
  {
    code: "02003",
    name: "الحجاج",
    state_code: "02",
  },
  {
    code: "02004",
    name: "أولاد بن عبد القادر",
    state_code: "02",
  },
  {
    code: "02005",
    name: "عين مران",
    state_code: "02",
  },
  {
    code: "02006",
    name: "بريرة",
    state_code: "02",
  },
  {
    code: "02007",
    name: "أولاد عباس",
    state_code: "02",
  },
  {
    code: "02008",
    name: "وادي الفضة",
    state_code: "02",
  },
  {
    code: "02009",
    name: "بني راشد",
    state_code: "02",
  },
  {
    code: "02010",
    name: "الهرانفة",
    state_code: "02",
  },
  {
    code: "02011",
    name: "تاجنة",
    state_code: "02",
  },
  {
    code: "02012",
    name: "المرسى",
    state_code: "02",
  },
  {
    code: "02013",
    name: "الشلف",
    state_code: "02",
  },
  {
    code: "02014",
    name: "أم الدروع",
    state_code: "02",
  },
  {
    code: "02015",
    name: "سنجاس",
    state_code: "02",
  },
  {
    code: "02016",
    name: "سيدي عبد الرحمن",
    state_code: "02",
  },
  {
    code: "02017",
    name: "سيدي عكاشة",
    state_code: "02",
  },
  {
    code: "02018",
    name: "تنس",
    state_code: "02",
  },
  {
    code: "02019",
    name: "بني بوعتاب",
    state_code: "02",
  },
  {
    code: "02020",
    name: "الكريمية",
    state_code: "02",
  },
  {
    code: "02021",
    name: "حرشون",
    state_code: "02",
  },
  {
    code: "02022",
    name: "بوزغاية",
    state_code: "02",
  },
  {
    code: "02023",
    name: "تاوقريت",
    state_code: "02",
  },
  {
    code: "02024",
    name: "بني حواء",
    state_code: "02",
  },
  {
    code: "02025",
    name: "أبو الحسن",
    state_code: "02",
  },
  {
    code: "02026",
    name: "وادي قوسين",
    state_code: "02",
  },
  {
    code: "02027",
    name: "الشطية",
    state_code: "02",
  },
  {
    code: "02028",
    name: "مصدق",
    state_code: "02",
  },
  {
    code: "02029",
    name: "أولاد فارس",
    state_code: "02",
  },
  {
    code: "02030",
    name: "بوقادير",
    state_code: "02",
  },
  {
    code: "02031",
    name: "وادي سلي",
    state_code: "02",
  },
  {
    code: "02032",
    name: "الصبحة",
    state_code: "02",
  },
  {
    code: "02033",
    name: "بنايرية",
    state_code: "02",
  },
  {
    code: "02034",
    name: "الأبيض مجاجة",
    state_code: "02",
  },
  {
    code: "02035",
    name: "الظهرة",
    state_code: "02",
  },
  {
    code: "03001",
    name: "البيضاء",
    state_code: "03",
  },
  {
    code: "03002",
    name: "قلتة سيدي سعد",
    state_code: "03",
  },
  {
    code: "03003",
    name: "بريدة",
    state_code: "03",
  },
  {
    code: "03004",
    name: "عين سيدي علي",
    state_code: "03",
  },
  {
    code: "03005",
    name: "تاجموت",
    state_code: "03",
  },
  {
    code: "03006",
    name: "الحاج مشري",
    state_code: "03",
  },
  {
    code: "03007",
    name: "تاويالة",
    state_code: "03",
  },
  {
    code: "03008",
    name: "الغيشة",
    state_code: "03",
  },
  {
    code: "03009",
    name: "تاجرونة",
    state_code: "03",
  },
  {
    code: "03010",
    name: "سبقاق",
    state_code: "03",
  },
  {
    code: "03011",
    name: "سيدي بوزيد",
    state_code: "03",
  },
  {
    code: "03012",
    name: "وادي مرة",
    state_code: "03",
  },
  {
    code: "03013",
    name: "الأغواط",
    state_code: "03",
  },
  {
    code: "03014",
    name: "وادي مزي",
    state_code: "03",
  },
  {
    code: "03015",
    name: "قصر الحيران",
    state_code: "03",
  },
  {
    code: "03016",
    name: "العسافية",
    state_code: "03",
  },
  {
    code: "03017",
    name: "سيدي مخلوف",
    state_code: "03",
  },
  {
    code: "03018",
    name: "حاسي الدلاعة",
    state_code: "03",
  },
  {
    code: "03019",
    name: "حاسي الرمل",
    state_code: "03",
  },
  {
    code: "03020",
    name: "عين ماضي",
    state_code: "03",
  },
  {
    code: "03021",
    name: "الحويطة",
    state_code: "03",
  },
  {
    code: "03022",
    name: "الخنق",
    state_code: "03",
  },
  {
    code: "03023",
    name: "بن ناصر بن شهرة",
    state_code: "03",
  },
  {
    code: "03024",
    name: "أفلو",
    state_code: "03",
  },
  {
    code: "04001",
    name: "فكيرينة",
    state_code: "04",
  },
  {
    code: "04002",
    name: "الفجوج بوغرارة سعودي",
    state_code: "04",
  },
  {
    code: "04003",
    name: "عين فكرون",
    state_code: "04",
  },
  {
    code: "04004",
    name: "الرحية",
    state_code: "04",
  },
  {
    code: "04005",
    name: "مسكيانة",
    state_code: "04",
  },
  {
    code: "04006",
    name: "البلالة",
    state_code: "04",
  },
  {
    code: "04007",
    name: "بحير الشرقي",
    state_code: "04",
  },
  {
    code: "04008",
    name: "قصر الصباحي",
    state_code: "04",
  },
  {
    code: "04009",
    name: "سوق نعمان",
    state_code: "04",
  },
  {
    code: "04010",
    name: "أولاد زواي",
    state_code: "04",
  },
  {
    code: "04011",
    name: "أم البواقي",
    state_code: "04",
  },
  {
    code: "04012",
    name: "عين ببوش",
    state_code: "04",
  },
  {
    code: "04013",
    name: "عين الزيتون",
    state_code: "04",
  },
  {
    code: "04014",
    name: "بئر الشهداء",
    state_code: "04",
  },
  {
    code: "04015",
    name: "عين البيضاء",
    state_code: "04",
  },
  {
    code: "04016",
    name: "بريش",
    state_code: "04",
  },
  {
    code: "04017",
    name: "الزرق",
    state_code: "04",
  },
  {
    code: "04018",
    name: "عين مليلة",
    state_code: "04",
  },
  {
    code: "04019",
    name: "أولاد قاسم",
    state_code: "04",
  },
  {
    code: "04020",
    name: "أولاد حملة",
    state_code: "04",
  },
  {
    code: "04021",
    name: "العامرية",
    state_code: "04",
  },
  {
    code: "04022",
    name: "سيقوس",
    state_code: "04",
  },
  {
    code: "04023",
    name: "وادي نيني",
    state_code: "04",
  },
  {
    code: "04024",
    name: "عين الديس",
    state_code: "04",
  },
  {
    code: "04025",
    name: "الضلعة",
    state_code: "04",
  },
  {
    code: "04026",
    name: "الجازية",
    state_code: "04",
  },
  {
    code: "04027",
    name: "عين كرشة",
    state_code: "04",
  },
  {
    code: "04028",
    name: "الحرملية",
    state_code: "04",
  },
  {
    code: "04029",
    name: "هنشير تومغني",
    state_code: "04",
  },
  {
    code: "05001",
    name: "معافة",
    state_code: "05",
  },
  {
    code: "05002",
    name: "القصبات",
    state_code: "05",
  },
  {
    code: "05003",
    name: "تيمقاد",
    state_code: "05",
  },
  {
    code: "05004",
    name: "تاكسلانت",
    state_code: "05",
  },
  {
    code: "05005",
    name: "أولاد سي سليمان",
    state_code: "05",
  },
  {
    code: "05006",
    name: "لمسان",
    state_code: "05",
  },
  {
    code: "05007",
    name: "تالخمت",
    state_code: "05",
  },
  {
    code: "05008",
    name: "رأس العيون",
    state_code: "05",
  },
  {
    code: "05009",
    name: "الرحبات",
    state_code: "05",
  },
  {
    code: "05010",
    name: "أولاد سلام",
    state_code: "05",
  },
  {
    code: "05011",
    name: "القيقبة",
    state_code: "05",
  },
  {
    code: "05012",
    name: "ثنية العابد",
    state_code: "05",
  },
  {
    code: "05013",
    name: "باتنة",
    state_code: "05",
  },
  {
    code: "05014",
    name: "فسديس",
    state_code: "05",
  },
  {
    code: "05015",
    name: "وادي الشعبة",
    state_code: "05",
  },
  {
    code: "05016",
    name: "حيدوسة",
    state_code: "05",
  },
  {
    code: "05017",
    name: "قصر بلزمة",
    state_code: "05",
  },
  {
    code: "05018",
    name: "مروانة",
    state_code: "05",
  },
  {
    code: "05019",
    name: "وادي الماء",
    state_code: "05",
  },
  {
    code: "05020",
    name: "لازرو",
    state_code: "05",
  },
  {
    code: "05021",
    name: "سريانة",
    state_code: "05",
  },
  {
    code: "05022",
    name: "زانة البيضاء",
    state_code: "05",
  },
  {
    code: "05023",
    name: "منعة",
    state_code: "05",
  },
  {
    code: "05024",
    name: "تغرغار",
    state_code: "05",
  },
  {
    code: "05025",
    name: "عين ياقوت",
    state_code: "05",
  },
  {
    code: "05026",
    name: "بومية",
    state_code: "05",
  },
  {
    code: "05027",
    name: "جرمة",
    state_code: "05",
  },
  {
    code: "05028",
    name: "المعذر",
    state_code: "05",
  },
  {
    code: "05029",
    name: "عيون العصافير",
    state_code: "05",
  },
  {
    code: "05030",
    name: "تازولت",
    state_code: "05",
  },
  {
    code: "05031",
    name: "بومقر",
    state_code: "05",
  },
  {
    code: "05032",
    name: "نقاوس",
    state_code: "05",
  },
  {
    code: "05033",
    name: "سفيان",
    state_code: "05",
  },
  {
    code: "05034",
    name: "أريس",
    state_code: "05",
  },
  {
    code: "05035",
    name: "تيغانمين",
    state_code: "05",
  },
  {
    code: "05036",
    name: "عين جاسر",
    state_code: "05",
  },
  {
    code: "05037",
    name: "الحاسي",
    state_code: "05",
  },
  {
    code: "05038",
    name: "سقانة",
    state_code: "05",
  },
  {
    code: "05039",
    name: "تيلاطو",
    state_code: "05",
  },
  {
    code: "05040",
    name: "فم الطوب",
    state_code: "05",
  },
  {
    code: "05041",
    name: "إشمول",
    state_code: "05",
  },
  {
    code: "05042",
    name: "إينوغيسن",
    state_code: "05",
  },
  {
    code: "05043",
    name: "بوزينة",
    state_code: "05",
  },
  {
    code: "05044",
    name: "لارباع",
    state_code: "05",
  },
  {
    code: "05045",
    name: "بولهيلات",
    state_code: "05",
  },
  {
    code: "05046",
    name: "الشمرة",
    state_code: "05",
  },
  {
    code: "05047",
    name: "بريكة",
    state_code: "05",
  },
  {
    code: "05048",
    name: "بيطام",
    state_code: "05",
  },
  {
    code: "05049",
    name: "إمدوكل",
    state_code: "05",
  },
  {
    code: "05050",
    name: "عزيل عبد القادر",
    state_code: "05",
  },
  {
    code: "05051",
    name: "الجزار",
    state_code: "05",
  },
  {
    code: "05052",
    name: "أولاد عمار",
    state_code: "05",
  },
  {
    code: "05053",
    name: "غسيرة",
    state_code: "05",
  },
  {
    code: "05054",
    name: "كيمل",
    state_code: "05",
  },
  {
    code: "05055",
    name: "تكوت",
    state_code: "05",
  },
  {
    code: "05056",
    name: "عين التوتة",
    state_code: "05",
  },
  {
    code: "05057",
    name: "بني فضالة الحقانية",
    state_code: "05",
  },
  {
    code: "05058",
    name: "أولاد فاضل",
    state_code: "05",
  },
  {
    code: "05059",
    name: "أولاد عوف",
    state_code: "05",
  },
  {
    code: "05060",
    name: "شير",
    state_code: "05",
  },
  {
    code: "05061",
    name: "وادي الطاقة",
    state_code: "05",
  },
  {
    code: "06001",
    name: "سيدي عياد",
    state_code: "06",
  },
  {
    code: "06002",
    name: "برباشة",
    state_code: "06",
  },
  {
    code: "06003",
    name: "الفلاي",
    state_code: "06",
  },
  {
    code: "06004",
    name: "كنديرة",
    state_code: "06",
  },
  {
    code: "06005",
    name: "سيدي عيش",
    state_code: "06",
  },
  {
    code: "06006",
    name: "تيفرة",
    state_code: "06",
  },
  {
    code: "06007",
    name: "تينبدار",
    state_code: "06",
  },
  {
    code: "06008",
    name: "القصر",
    state_code: "06",
  },
  {
    code: "06009",
    name: "فناية الماثن",
    state_code: "06",
  },
  {
    code: "06010",
    name: "توجة",
    state_code: "06",
  },
  {
    code: "06011",
    name: "ذراع القايد",
    state_code: "06",
  },
  {
    code: "06012",
    name: "خراطة",
    state_code: "06",
  },
  {
    code: "06013",
    name: "بجاية",
    state_code: "06",
  },
  {
    code: "06014",
    name: "وادي غير",
    state_code: "06",
  },
  {
    code: "06015",
    name: "بني معوش",
    state_code: "06",
  },
  {
    code: "06016",
    name: "بني جليل",
    state_code: "06",
  },
  {
    code: "06017",
    name: "فرعون",
    state_code: "06",
  },
  {
    code: "06018",
    name: "سمعون",
    state_code: "06",
  },
  {
    code: "06019",
    name: "تيمزريت",
    state_code: "06",
  },
  {
    code: "06020",
    name: "مالبو",
    state_code: "06",
  },
  {
    code: "06021",
    name: "سوق لإثنين",
    state_code: "06",
  },
  {
    code: "06022",
    name: "تامريجت",
    state_code: "06",
  },
  {
    code: "06023",
    name: "بوخليفة",
    state_code: "06",
  },
  {
    code: "06024",
    name: "تالة حمزة",
    state_code: "06",
  },
  {
    code: "06025",
    name: "تيشي",
    state_code: "06",
  },
  {
    code: "06026",
    name: "أيت رزين",
    state_code: "06",
  },
  {
    code: "06027",
    name: "إغيل علي",
    state_code: "06",
  },
  {
    code: "06028",
    name: "أيت إسماعيل",
    state_code: "06",
  },
  {
    code: "06029",
    name: "درقينة",
    state_code: "06",
  },
  {
    code: "06030",
    name: "تاسكريوت",
    state_code: "06",
  },
  {
    code: "06031",
    name: "أوقاس",
    state_code: "06",
  },
  {
    code: "06032",
    name: "تيزي نبربر",
    state_code: "06",
  },
  {
    code: "06033",
    name: "أدكار",
    state_code: "06",
  },
  {
    code: "06034",
    name: "بني كسيلة",
    state_code: "06",
  },
  {
    code: "06035",
    name: "تاوريرت إغيل",
    state_code: "06",
  },
  {
    code: "06036",
    name: "أقبو",
    state_code: "06",
  },
  {
    code: "06037",
    name: "شلاطة",
    state_code: "06",
  },
  {
    code: "06038",
    name: "اغرم",
    state_code: "06",
  },
  {
    code: "06039",
    name: "تامقرة",
    state_code: "06",
  },
  {
    code: "06040",
    name: "أمالو",
    state_code: "06",
  },
  {
    code: "06041",
    name: "بوحمزة",
    state_code: "06",
  },
  {
    code: "06042",
    name: "مسيسنة",
    state_code: "06",
  },
  {
    code: "06043",
    name: "صدوق",
    state_code: "06",
  },
  {
    code: "06044",
    name: "بني مليكش",
    state_code: "06",
  },
  {
    code: "06045",
    name: "بو جليل",
    state_code: "06",
  },
  {
    code: "06046",
    name: "تازمالت",
    state_code: "06",
  },
  {
    code: "06047",
    name: "أكفادو",
    state_code: "06",
  },
  {
    code: "06048",
    name: "شميني",
    state_code: "06",
  },
  {
    code: "06049",
    name: "سوق اوفلا",
    state_code: "06",
  },
  {
    code: "06050",
    name: "طيبان",
    state_code: "06",
  },
  {
    code: "06051",
    name: "أوزلاقن",
    state_code: "06",
  },
  {
    code: "06052",
    name: "أميزور",
    state_code: "06",
  },
  {
    code: "07001",
    name: "الفيض",
    state_code: "07",
  },
  {
    code: "07002",
    name: "ليشانة",
    state_code: "07",
  },
  {
    code: "07003",
    name: "بوشقرون",
    state_code: "07",
  },
  {
    code: "07004",
    name: "مخادمة",
    state_code: "07",
  },
  {
    code: "07005",
    name: "جمورة",
    state_code: "07",
  },
  {
    code: "07006",
    name: "برانيس",
    state_code: "07",
  },
  {
    code: "07007",
    name: "الوطاية",
    state_code: "07",
  },
  {
    code: "07008",
    name: "القنطرة",
    state_code: "07",
  },
  {
    code: "07009",
    name: "خنقة سيدي ناجي",
    state_code: "07",
  },
  {
    code: "07010",
    name: "عين زعطوط",
    state_code: "07",
  },
  {
    code: "07011",
    name: "زريبة الوادي",
    state_code: "07",
  },
  {
    code: "07012",
    name: "المزيرعة",
    state_code: "07",
  },
  {
    code: "07013",
    name: "بسكرة",
    state_code: "07",
  },
  {
    code: "07014",
    name: "الحاجب",
    state_code: "07",
  },
  {
    code: "07015",
    name: "مليلي",
    state_code: "07",
  },
  {
    code: "07016",
    name: "فوغالة",
    state_code: "07",
  },
  {
    code: "07017",
    name: "الغروس",
    state_code: "07",
  },
  {
    code: "07018",
    name: "برج بن عزوز",
    state_code: "07",
  },
  {
    code: "07019",
    name: "أورلال",
    state_code: "07",
  },
  {
    code: "07020",
    name: "أوماش",
    state_code: "07",
  },
  {
    code: "07021",
    name: "عين الناقة",
    state_code: "07",
  },
  {
    code: "07022",
    name: "شتمة",
    state_code: "07",
  },
  {
    code: "07023",
    name: "الحوش",
    state_code: "07",
  },
  {
    code: "07024",
    name: "سيدي عقبة",
    state_code: "07",
  },
  {
    code: "07025",
    name: "مشونش",
    state_code: "07",
  },
  {
    code: "07026",
    name: "ليوة",
    state_code: "07",
  },
  {
    code: "07027",
    name: "طولقة",
    state_code: "07",
  },
  {
    code: "08001",
    name: "بشار",
    state_code: "08",
  },
  {
    code: "08002",
    name: "بوكايس",
    state_code: "08",
  },
  {
    code: "08003",
    name: "لحمر",
    state_code: "08",
  },
  {
    code: "08004",
    name: "موغل",
    state_code: "08",
  },
  {
    code: "08005",
    name: "المريجة",
    state_code: "08",
  },
  {
    code: "08006",
    name: "تاغيت",
    state_code: "08",
  },
  {
    code: "08007",
    name: "العبادلة",
    state_code: "08",
  },
  {
    code: "08008",
    name: "عرق فراج",
    state_code: "08",
  },
  {
    code: "08009",
    name: "مشرع هواري بومدين",
    state_code: "08",
  },
  {
    code: "08010",
    name: "بني ونيف",
    state_code: "08",
  },
  {
    code: "08011",
    name: "تبلبالة",
    state_code: "08",
  },
  {
    code: "08012",
    name: "القنادسة",
    state_code: "08",
  },
  {
    code: "09001",
    name: "بني مراد",
    state_code: "09",
  },
  {
    code: "09002",
    name: "اولاد سلامة",
    state_code: "09",
  },
  {
    code: "09003",
    name: "موزاية",
    state_code: "09",
  },
  {
    code: "09004",
    name: "حمام ملوان",
    state_code: "09",
  },
  {
    code: "09005",
    name: "بوقرة",
    state_code: "09",
  },
  {
    code: "09006",
    name: "صوحان",
    state_code: "09",
  },
  {
    code: "09007",
    name: "الأربعاء",
    state_code: "09",
  },
  {
    code: "09008",
    name: "الصومعة",
    state_code: "09",
  },
  {
    code: "09009",
    name: "قرواو",
    state_code: "09",
  },
  {
    code: "09010",
    name: "بوفاريك",
    state_code: "09",
  },
  {
    code: "09011",
    name: "مفتاح",
    state_code: "09",
  },
  {
    code: "09012",
    name: "الشفة",
    state_code: "09",
  },
  {
    code: "09013",
    name: "عين الرمانة",
    state_code: "09",
  },
  {
    code: "09014",
    name: "وادي جر",
    state_code: "09",
  },
  {
    code: "09015",
    name: "العفرون",
    state_code: "09",
  },
  {
    code: "09016",
    name: "أولاد يعيش",
    state_code: "09",
  },
  {
    code: "09017",
    name: "الشريعة",
    state_code: "09",
  },
  {
    code: "09018",
    name: "جبابرة",
    state_code: "09",
  },
  {
    code: "09019",
    name: "وادي العلايق",
    state_code: "09",
  },
  {
    code: "09020",
    name: "بن خليل",
    state_code: "09",
  },
  {
    code: "09021",
    name: "بني تامو",
    state_code: "09",
  },
  {
    code: "09022",
    name: "الشبلي",
    state_code: "09",
  },
  {
    code: "09023",
    name: "بوعينان",
    state_code: "09",
  },
  {
    code: "09024",
    name: "بوعرفة",
    state_code: "09",
  },
  {
    code: "09025",
    name: "البليدة",
    state_code: "09",
  },
  {
    code: "10001",
    name: "عين العلوي",
    state_code: "10",
  },
  {
    code: "10002",
    name: "الحجرة الزرقاء",
    state_code: "10",
  },
  {
    code: "10003",
    name: "مزدور",
    state_code: "10",
  },
  {
    code: "10004",
    name: "تاقديت",
    state_code: "10",
  },
  {
    code: "10005",
    name: "ريدان",
    state_code: "10",
  },
  {
    code: "10006",
    name: "المعمورة",
    state_code: "10",
  },
  {
    code: "10007",
    name: "الحاكمية",
    state_code: "10",
  },
  {
    code: "10008",
    name: "أهل القصر",
    state_code: "10",
  },
  {
    code: "10009",
    name: "ديرة",
    state_code: "10",
  },
  {
    code: "10010",
    name: "الدشمية",
    state_code: "10",
  },
  {
    code: "10011",
    name: "بشلول",
    state_code: "10",
  },
  {
    code: "10012",
    name: "آث  منصور",
    state_code: "10",
  },
  {
    code: "10013",
    name: "سحاريج",
    state_code: "10",
  },
  {
    code: "10014",
    name: "العجيبة",
    state_code: "10",
  },
  {
    code: "10015",
    name: "الأسنام",
    state_code: "10",
  },
  {
    code: "10016",
    name: "أمشدالة",
    state_code: "10",
  },
  {
    code: "10017",
    name: "برج أوخريص",
    state_code: "10",
  },
  {
    code: "10018",
    name: "سور الغزلان",
    state_code: "10",
  },
  {
    code: "10019",
    name: "حنيف",
    state_code: "10",
  },
  {
    code: "10020",
    name: "شرفة",
    state_code: "10",
  },
  {
    code: "10021",
    name: "أولاد راشد",
    state_code: "10",
  },
  {
    code: "10022",
    name: "عين الحجر",
    state_code: "10",
  },
  {
    code: "10023",
    name: "أغبالو",
    state_code: "10",
  },
  {
    code: "10024",
    name: "روراوة",
    state_code: "10",
  },
  {
    code: "10025",
    name: "الخبوزية",
    state_code: "10",
  },
  {
    code: "10026",
    name: "بئر غبالو",
    state_code: "10",
  },
  {
    code: "10027",
    name: "البويرة",
    state_code: "10",
  },
  {
    code: "10028",
    name: "عين الترك",
    state_code: "10",
  },
  {
    code: "10029",
    name: "أيت لعزيز",
    state_code: "10",
  },
  {
    code: "10030",
    name: "عين بسام",
    state_code: "10",
  },
  {
    code: "10031",
    name: "المقراني",
    state_code: "10",
  },
  {
    code: "10032",
    name: "سوق الخميس",
    state_code: "10",
  },
  {
    code: "10033",
    name: "أعمر",
    state_code: "10",
  },
  {
    code: "10034",
    name: "جباحية",
    state_code: "10",
  },
  {
    code: "10035",
    name: "الهاشمية",
    state_code: "10",
  },
  {
    code: "10036",
    name: "حيزر",
    state_code: "10",
  },
  {
    code: "10037",
    name: "تاغزوت",
    state_code: "10",
  },
  {
    code: "10038",
    name: "بودربالة",
    state_code: "10",
  },
  {
    code: "10039",
    name: "بوكرم",
    state_code: "10",
  },
  {
    code: "10040",
    name: "قرومة",
    state_code: "10",
  },
  {
    code: "10041",
    name: "الأخضرية",
    state_code: "10",
  },
  {
    code: "10042",
    name: "معلة",
    state_code: "10",
  },
  {
    code: "10043",
    name: "قادرية",
    state_code: "10",
  },
  {
    code: "10044",
    name: "زبربر",
    state_code: "10",
  },
  {
    code: "10045",
    name: "وادي البردي",
    state_code: "10",
  },
  {
    code: "11001",
    name: "تاظروك",
    state_code: "11",
  },
  {
    code: "11002",
    name: "ابلسة",
    state_code: "11",
  },
  {
    code: "11003",
    name: "تمنراست",
    state_code: "11",
  },
  {
    code: "11004",
    name: "عين امقل",
    state_code: "11",
  },
  {
    code: "11005",
    name: "أدلس",
    state_code: "11",
  },
  {
    code: "12001",
    name: "الحويجبات",
    state_code: "12",
  },
  {
    code: "12002",
    name: "العوينات",
    state_code: "12",
  },
  {
    code: "12003",
    name: "فركان",
    state_code: "12",
  },
  {
    code: "12004",
    name: "نقرين",
    state_code: "12",
  },
  {
    code: "12005",
    name: "بئر مقدم",
    state_code: "12",
  },
  {
    code: "12006",
    name: "بئر الذهب",
    state_code: "12",
  },
  {
    code: "12007",
    name: "صفصاف الوسرى",
    state_code: "12",
  },
  {
    code: "12008",
    name: "قريقر",
    state_code: "12",
  },
  {
    code: "12009",
    name: "بكارية",
    state_code: "12",
  },
  {
    code: "12010",
    name: "بولحاف الدير",
    state_code: "12",
  },
  {
    code: "12011",
    name: "أم علي",
    state_code: "12",
  },
  {
    code: "12012",
    name: "بوخضرة",
    state_code: "12",
  },
  {
    code: "12013",
    name: "الماء الابيض",
    state_code: "12",
  },
  {
    code: "12014",
    name: "الونزة",
    state_code: "12",
  },
  {
    code: "12015",
    name: "المريج",
    state_code: "12",
  },
  {
    code: "12016",
    name: "عين الزرقاء",
    state_code: "12",
  },
  {
    code: "12017",
    name: "سطح قنطيس",
    state_code: "12",
  },
  {
    code: "12018",
    name: "العقلة",
    state_code: "12",
  },
  {
    code: "12019",
    name: "المزرعة",
    state_code: "12",
  },
  {
    code: "12020",
    name: "بجن",
    state_code: "12",
  },
  {
    code: "12021",
    name: "مرسط",
    state_code: "12",
  },
  {
    code: "12022",
    name: "ثليجان",
    state_code: "12",
  },
  {
    code: "12023",
    name: "الشريعة",
    state_code: "12",
  },
  {
    code: "12024",
    name: "العقلة المالحة",
    state_code: "12",
  },
  {
    code: "12025",
    name: "بئر العاتر",
    state_code: "12",
  },
  {
    code: "12026",
    name: "تبسة",
    state_code: "12",
  },
  {
    code: "12027",
    name: "الحمامات",
    state_code: "12",
  },
  {
    code: "12028",
    name: "الكويف",
    state_code: "12",
  },
  {
    code: "13001",
    name: "باب العسة",
    state_code: "13",
  },
  {
    code: "13002",
    name: "تيرني بني هديل",
    state_code: "13",
  },
  {
    code: "13003",
    name: "منصورة",
    state_code: "13",
  },
  {
    code: "13004",
    name: "بني مستر",
    state_code: "13",
  },
  {
    code: "13005",
    name: "عين غرابة",
    state_code: "13",
  },
  {
    code: "13006",
    name: "شتوان",
    state_code: "13",
  },
  {
    code: "13007",
    name: "عمير",
    state_code: "13",
  },
  {
    code: "13008",
    name: "عين فزة",
    state_code: "13",
  },
  {
    code: "13009",
    name: "هنين",
    state_code: "13",
  },
  {
    code: "13010",
    name: "بني خلاد",
    state_code: "13",
  },
  {
    code: "13011",
    name: "سيدي الجيلالي",
    state_code: "13",
  },
  {
    code: "13012",
    name: "البويهي",
    state_code: "13",
  },
  {
    code: "13013",
    name: "ندرومة",
    state_code: "13",
  },
  {
    code: "13014",
    name: "مسيردة الفواقة",
    state_code: "13",
  },
  {
    code: "13015",
    name: "مرسى بن مهيدي",
    state_code: "13",
  },
  {
    code: "13016",
    name: "سيدي مجاهد",
    state_code: "13",
  },
  {
    code: "13017",
    name: "بني بوسعيد",
    state_code: "13",
  },
  {
    code: "13018",
    name: "سبدو",
    state_code: "13",
  },
  {
    code: "13019",
    name: "القور",
    state_code: "13",
  },
  {
    code: "13020",
    name: "العريشة",
    state_code: "13",
  },
  {
    code: "13021",
    name: "بوحلو",
    state_code: "13",
  },
  {
    code: "13022",
    name: "مغنية",
    state_code: "13",
  },
  {
    code: "13023",
    name: "حمام بوغرارة",
    state_code: "13",
  },
  {
    code: "13024",
    name: "زناتة",
    state_code: "13",
  },
  {
    code: "13025",
    name: "أولاد رياح",
    state_code: "13",
  },
  {
    code: "13026",
    name: "الحناية",
    state_code: "13",
  },
  {
    code: "13027",
    name: "سيدي العبدلي",
    state_code: "13",
  },
  {
    code: "13028",
    name: "سوق الثلاثاء",
    state_code: "13",
  },
  {
    code: "13029",
    name: "بن سكران",
    state_code: "13",
  },
  {
    code: "13030",
    name: "فلاوسن",
    state_code: "13",
  },
  {
    code: "13031",
    name: "عين الكبيرة",
    state_code: "13",
  },
  {
    code: "13032",
    name: "عين فتاح",
    state_code: "13",
  },
  {
    code: "13033",
    name: "تلمسان",
    state_code: "13",
  },
  {
    code: "13034",
    name: "عين النحالة",
    state_code: "13",
  },
  {
    code: "13035",
    name: "عين تالوت",
    state_code: "13",
  },
  {
    code: "13036",
    name: "عين يوسف",
    state_code: "13",
  },
  {
    code: "13037",
    name: "بني وارسوس",
    state_code: "13",
  },
  {
    code: "13038",
    name: "الفحول",
    state_code: "13",
  },
  {
    code: "13039",
    name: "الرمشي",
    state_code: "13",
  },
  {
    code: "13040",
    name: "سبعة شيوخ",
    state_code: "13",
  },
  {
    code: "13041",
    name: "السواني",
    state_code: "13",
  },
  {
    code: "13042",
    name: "صبرة",
    state_code: "13",
  },
  {
    code: "13043",
    name: "دار يغمراسن",
    state_code: "13",
  },
  {
    code: "13044",
    name: "الغزوات",
    state_code: "13",
  },
  {
    code: "13045",
    name: "السواحلية",
    state_code: "13",
  },
  {
    code: "13046",
    name: "تيانت",
    state_code: "13",
  },
  {
    code: "13047",
    name: "بني صميل",
    state_code: "13",
  },
  {
    code: "13048",
    name: "وادي الخضر",
    state_code: "13",
  },
  {
    code: "13049",
    name: "أولاد ميمون",
    state_code: "13",
  },
  {
    code: "13050",
    name: "بني بهدل",
    state_code: "13",
  },
  {
    code: "13051",
    name: "بني سنوس",
    state_code: "13",
  },
  {
    code: "13052",
    name: "العزايل",
    state_code: "13",
  },
  {
    code: "13053",
    name: "جبالة",
    state_code: "13",
  },
  {
    code: "14001",
    name: "مهدية",
    state_code: "14",
  },
  {
    code: "14002",
    name: "عين دزاريت",
    state_code: "14",
  },
  {
    code: "14003",
    name: "السبعين",
    state_code: "14",
  },
  {
    code: "14004",
    name: "الفايجة",
    state_code: "14",
  },
  {
    code: "14005",
    name: "سي عبد الغني",
    state_code: "14",
  },
  {
    code: "14006",
    name: "السوقر",
    state_code: "14",
  },
  {
    code: "14007",
    name: "توسنينة",
    state_code: "14",
  },
  {
    code: "14008",
    name: "مغيلة",
    state_code: "14",
  },
  {
    code: "14009",
    name: "السبت",
    state_code: "14",
  },
  {
    code: "14010",
    name: "سيدي حسني",
    state_code: "14",
  },
  {
    code: "14011",
    name: "عين الحديد",
    state_code: "14",
  },
  {
    code: "14012",
    name: "فرندة",
    state_code: "14",
  },
  {
    code: "14013",
    name: "تخمرت",
    state_code: "14",
  },
  {
    code: "14014",
    name: "عين كرمس",
    state_code: "14",
  },
  {
    code: "14015",
    name: "جبيلات الرصفاء",
    state_code: "14",
  },
  {
    code: "14016",
    name: "مادنة",
    state_code: "14",
  },
  {
    code: "14017",
    name: "مدريسة",
    state_code: "14",
  },
  {
    code: "14018",
    name: "سيدي عبد الرحمن",
    state_code: "14",
  },
  {
    code: "14019",
    name: "قصر الشلالة",
    state_code: "14",
  },
  {
    code: "14020",
    name: "قرطوفة",
    state_code: "14",
  },
  {
    code: "14021",
    name: "سرغين",
    state_code: "14",
  },
  {
    code: "14022",
    name: "زمالة  الأمير عبد القادر",
    state_code: "14",
  },
  {
    code: "14023",
    name: "وادي ليلي",
    state_code: "14",
  },
  {
    code: "14024",
    name: "سيدي علي ملال",
    state_code: "14",
  },
  {
    code: "14025",
    name: "جيلالي بن عمار",
    state_code: "14",
  },
  {
    code: "14026",
    name: "مشرع الصفا",
    state_code: "14",
  },
  {
    code: "14027",
    name: "تاقدمت",
    state_code: "14",
  },
  {
    code: "14028",
    name: "بوقرة",
    state_code: "14",
  },
  {
    code: "14029",
    name: "حمادية",
    state_code: "14",
  },
  {
    code: "14030",
    name: "الرشايقة",
    state_code: "14",
  },
  {
    code: "14031",
    name: "تيدة",
    state_code: "14",
  },
  {
    code: "14032",
    name: "الناظورة",
    state_code: "14",
  },
  {
    code: "14033",
    name: "تيارت",
    state_code: "14",
  },
  {
    code: "14034",
    name: "مدروسة",
    state_code: "14",
  },
  {
    code: "14035",
    name: "ملاكو",
    state_code: "14",
  },
  {
    code: "14036",
    name: "سيدي بختي",
    state_code: "14",
  },
  {
    code: "14037",
    name: "عين الذهب",
    state_code: "14",
  },
  {
    code: "14038",
    name: "شحيمة",
    state_code: "14",
  },
  {
    code: "14039",
    name: "النعيمة",
    state_code: "14",
  },
  {
    code: "14040",
    name: "عين بوشقيف",
    state_code: "14",
  },
  {
    code: "14041",
    name: "دحموني",
    state_code: "14",
  },
  {
    code: "14042",
    name: "الرحوية",
    state_code: "14",
  },
  {
    code: "15001",
    name: "ميزرانـــة",
    state_code: "15",
  },
  {
    code: "15002",
    name: "إيجــار",
    state_code: "15",
  },
  {
    code: "15003",
    name: "بني دوالة",
    state_code: "15",
  },
  {
    code: "15004",
    name: "بني زيكــي",
    state_code: "15",
  },
  {
    code: "15005",
    name: "إيلولة أومـــالو",
    state_code: "15",
  },
  {
    code: "15006",
    name: "أقني قغران",
    state_code: "15",
  },
  {
    code: "15007",
    name: "أيت بــوادو",
    state_code: "15",
  },
  {
    code: "15008",
    name: "واضية",
    state_code: "15",
  },
  {
    code: "15009",
    name: "تيزي نثلاثة",
    state_code: "15",
  },
  {
    code: "15010",
    name: "أغريب",
    state_code: "15",
  },
  {
    code: "15011",
    name: "أيت شافع",
    state_code: "15",
  },
  {
    code: "15012",
    name: "أقرو",
    state_code: "15",
  },
  {
    code: "15013",
    name: "أزفون",
    state_code: "15",
  },
  {
    code: "15014",
    name: "إفليـــسن",
    state_code: "15",
  },
  {
    code: "15015",
    name: "تيقـزيرت",
    state_code: "15",
  },
  {
    code: "15016",
    name: "أسي يوسف",
    state_code: "15",
  },
  {
    code: "15017",
    name: "بوغني",
    state_code: "15",
  },
  {
    code: "15018",
    name: "بونوح",
    state_code: "15",
  },
  {
    code: "15019",
    name: "مشطراس",
    state_code: "15",
  },
  {
    code: "15020",
    name: "ذراع بن خدة",
    state_code: "15",
  },
  {
    code: "15021",
    name: "سيدي نعمان",
    state_code: "15",
  },
  {
    code: "15022",
    name: "تادمايت",
    state_code: "15",
  },
  {
    code: "15023",
    name: "تيرمتين",
    state_code: "15",
  },
  {
    code: "15024",
    name: "أيت بومهدي",
    state_code: "15",
  },
  {
    code: "15025",
    name: "أيت تودرت",
    state_code: "15",
  },
  {
    code: "15026",
    name: "بني عيسي",
    state_code: "15",
  },
  {
    code: "15027",
    name: "واسيف",
    state_code: "15",
  },
  {
    code: "15028",
    name: "أيت خليلي",
    state_code: "15",
  },
  {
    code: "15029",
    name: "مقــلع",
    state_code: "15",
  },
  {
    code: "15030",
    name: "صوامـــع",
    state_code: "15",
  },
  {
    code: "15031",
    name: "بني يني",
    state_code: "15",
  },
  {
    code: "15032",
    name: "إبودرارن",
    state_code: "15",
  },
  {
    code: "15033",
    name: "تيزي وزو",
    state_code: "15",
  },
  {
    code: "15034",
    name: "أبي يوسف",
    state_code: "15",
  },
  {
    code: "15035",
    name: "عين الحمام",
    state_code: "15",
  },
  {
    code: "15036",
    name: "أيت يحيى",
    state_code: "15",
  },
  {
    code: "15037",
    name: "اقبيل",
    state_code: "15",
  },
  {
    code: "15038",
    name: "بوجيمة",
    state_code: "15",
  },
  {
    code: "15039",
    name: "ماكودة",
    state_code: "15",
  },
  {
    code: "15040",
    name: "عين الزاوية",
    state_code: "15",
  },
  {
    code: "15041",
    name: "أيت يحي موسى",
    state_code: "15",
  },
  {
    code: "15042",
    name: "ذراع الميزان",
    state_code: "15",
  },
  {
    code: "15043",
    name: "فريقات",
    state_code: "15",
  },
  {
    code: "15044",
    name: "مكيرة",
    state_code: "15",
  },
  {
    code: "15045",
    name: "تيزي غنيف",
    state_code: "15",
  },
  {
    code: "15046",
    name: "يطــافن",
    state_code: "15",
  },
  {
    code: "15047",
    name: "إيلـيــلتـن",
    state_code: "15",
  },
  {
    code: "15048",
    name: "إمســوحال",
    state_code: "15",
  },
  {
    code: "15049",
    name: "عزازقة",
    state_code: "15",
  },
  {
    code: "15050",
    name: "فريحة",
    state_code: "15",
  },
  {
    code: "15051",
    name: "إيفيغاء",
    state_code: "15",
  },
  {
    code: "15052",
    name: "إعــكورن",
    state_code: "15",
  },
  {
    code: "15053",
    name: "زكري",
    state_code: "15",
  },
  {
    code: "15054",
    name: "أيت عقـواشة",
    state_code: "15",
  },
  {
    code: "15055",
    name: "إيرجـــن",
    state_code: "15",
  },
  {
    code: "15056",
    name: "الأربعــاء ناث إيراثن",
    state_code: "15",
  },
  {
    code: "15057",
    name: "أيت  أومالو",
    state_code: "15",
  },
  {
    code: "15058",
    name: "تيزي راشد",
    state_code: "15",
  },
  {
    code: "15059",
    name: "أيت عيسى ميمون",
    state_code: "15",
  },
  {
    code: "15060",
    name: "واقنون",
    state_code: "15",
  },
  {
    code: "15061",
    name: "تيمـيزار",
    state_code: "15",
  },
  {
    code: "15062",
    name: "معـــاتقة",
    state_code: "15",
  },
  {
    code: "15063",
    name: "سوق الإثنين",
    state_code: "15",
  },
  {
    code: "15064",
    name: "أيت محمود",
    state_code: "15",
  },
  {
    code: "15065",
    name: "بنــــي زمنزار",
    state_code: "15",
  },
  {
    code: "15066",
    name: "إفــرحــونان",
    state_code: "15",
  },
  {
    code: "15067",
    name: "بوزقــن",
    state_code: "15",
  },
  {
    code: "16001",
    name: "حسين داي",
    state_code: "16",
  },
  {
    code: "16002",
    name: "الكاليتوس",
    state_code: "16",
  },
  {
    code: "16003",
    name: "سيدي موسى",
    state_code: "16",
  },
  {
    code: "16004",
    name: "القبة",
    state_code: "16",
  },
  {
    code: "16005",
    name: "محمد بلوزداد",
    state_code: "16",
  },
  {
    code: "16006",
    name: "عين طاية",
    state_code: "16",
  },
  {
    code: "16007",
    name: "باب الزوار",
    state_code: "16",
  },
  {
    code: "16008",
    name: "برج الكيفان",
    state_code: "16",
  },
  {
    code: "16009",
    name: "الدار البيضاء",
    state_code: "16",
  },
  {
    code: "16010",
    name: "المرسى",
    state_code: "16",
  },
  {
    code: "16011",
    name: "المحمدية",
    state_code: "16",
  },
  {
    code: "16012",
    name: "بئر توتة",
    state_code: "16",
  },
  {
    code: "16013",
    name: "اولاد شبل",
    state_code: "16",
  },
  {
    code: "16014",
    name: "تسالة المرجة",
    state_code: "16",
  },
  {
    code: "16015",
    name: "هراوة",
    state_code: "16",
  },
  {
    code: "16016",
    name: "رغاية",
    state_code: "16",
  },
  {
    code: "16017",
    name: "الرويبة",
    state_code: "16",
  },
  {
    code: "16018",
    name: "المعالمة",
    state_code: "16",
  },
  {
    code: "16019",
    name: "الرحمانية",
    state_code: "16",
  },
  {
    code: "16020",
    name: "سويدانية",
    state_code: "16",
  },
  {
    code: "16021",
    name: "سطاوالي",
    state_code: "16",
  },
  {
    code: "16022",
    name: "زرالدة",
    state_code: "16",
  },
  {
    code: "16023",
    name: "بابا حسن",
    state_code: "16",
  },
  {
    code: "16024",
    name: "الدويرة",
    state_code: "16",
  },
  {
    code: "16025",
    name: "الدرارية",
    state_code: "16",
  },
  {
    code: "16026",
    name: "العاشور",
    state_code: "16",
  },
  {
    code: "16027",
    name: "الخرايسية",
    state_code: "16",
  },
  {
    code: "16028",
    name: "عين بنيان",
    state_code: "16",
  },
  {
    code: "16029",
    name: "الشراقة",
    state_code: "16",
  },
  {
    code: "16030",
    name: "دالي ابراهيم",
    state_code: "16",
  },
  {
    code: "16031",
    name: "الحمامات",
    state_code: "16",
  },
  {
    code: "16032",
    name: "اولاد فايت",
    state_code: "16",
  },
  {
    code: "16033",
    name: "الجزائر الوسطى",
    state_code: "16",
  },
  {
    code: "16034",
    name: "المدنية",
    state_code: "16",
  },
  {
    code: "16035",
    name: "المرادية",
    state_code: "16",
  },
  {
    code: "16036",
    name: "سيدي امحمد",
    state_code: "16",
  },
  {
    code: "16037",
    name: "السحاولة",
    state_code: "16",
  },
  {
    code: "16038",
    name: "بولوغين بن زيري",
    state_code: "16",
  },
  {
    code: "16039",
    name: "القصبة",
    state_code: "16",
  },
  {
    code: "16040",
    name: "وادي قريش",
    state_code: "16",
  },
  {
    code: "16041",
    name: "الرايس حميدو",
    state_code: "16",
  },
  {
    code: "16042",
    name: "بئر مراد رايس",
    state_code: "16",
  },
  {
    code: "16043",
    name: "بئر خادم",
    state_code: "16",
  },
  {
    code: "16044",
    name: "جسر قسنطينة",
    state_code: "16",
  },
  {
    code: "16045",
    name: "حيدرة",
    state_code: "16",
  },
  {
    code: "16046",
    name: "المغارية",
    state_code: "16",
  },
  {
    code: "16047",
    name: "ابن عكنون",
    state_code: "16",
  },
  {
    code: "16048",
    name: "بني مسوس",
    state_code: "16",
  },
  {
    code: "16049",
    name: "بوزريعة",
    state_code: "16",
  },
  {
    code: "16050",
    name: "الابيار",
    state_code: "16",
  },
  {
    code: "16051",
    name: "باش جراح",
    state_code: "16",
  },
  {
    code: "16052",
    name: "بوروبة",
    state_code: "16",
  },
  {
    code: "16053",
    name: "الحراش",
    state_code: "16",
  },
  {
    code: "16054",
    name: "وادي السمار",
    state_code: "16",
  },
  {
    code: "16055",
    name: "براقي",
    state_code: "16",
  },
  {
    code: "16056",
    name: "برج البحري",
    state_code: "16",
  },
  {
    code: "16057",
    name: "باب الوادي",
    state_code: "16",
  },
  {
    code: "17001",
    name: "حاسي العش",
    state_code: "17",
  },
  {
    code: "17002",
    name: "عين الإبل",
    state_code: "17",
  },
  {
    code: "17003",
    name: "القديد",
    state_code: "17",
  },
  {
    code: "17004",
    name: "الشارف",
    state_code: "17",
  },
  {
    code: "17005",
    name: "بن يعقوب",
    state_code: "17",
  },
  {
    code: "17006",
    name: "سيدي بايزيد",
    state_code: "17",
  },
  {
    code: "17007",
    name: "مليليحة",
    state_code: "17",
  },
  {
    code: "17008",
    name: "دار الشيوخ",
    state_code: "17",
  },
  {
    code: "17009",
    name: "تعظميت",
    state_code: "17",
  },
  {
    code: "17010",
    name: "حد الصحاري",
    state_code: "17",
  },
  {
    code: "17011",
    name: "بويرة الأحداب",
    state_code: "17",
  },
  {
    code: "17012",
    name: "عين فقه",
    state_code: "17",
  },
  {
    code: "17013",
    name: "سيدي لعجال",
    state_code: "17",
  },
  {
    code: "17014",
    name: "حاسي فدول",
    state_code: "17",
  },
  {
    code: "17015",
    name: "الخميس",
    state_code: "17",
  },
  {
    code: "17016",
    name: "سلمانة",
    state_code: "17",
  },
  {
    code: "17017",
    name: "سد الرحال",
    state_code: "17",
  },
  {
    code: "17018",
    name: "مسعد",
    state_code: "17",
  },
  {
    code: "17019",
    name: "قطارة",
    state_code: "17",
  },
  {
    code: "17020",
    name: "دلدول",
    state_code: "17",
  },
  {
    code: "17021",
    name: "زكار",
    state_code: "17",
  },
  {
    code: "17022",
    name: "دويس",
    state_code: "17",
  },
  {
    code: "17023",
    name: "الادريسية",
    state_code: "17",
  },
  {
    code: "17024",
    name: "عين الشهداء",
    state_code: "17",
  },
  {
    code: "17025",
    name: "الجلفة",
    state_code: "17",
  },
  {
    code: "17026",
    name: "بيرين",
    state_code: "17",
  },
  {
    code: "17027",
    name: "أم العظام",
    state_code: "17",
  },
  {
    code: "17028",
    name: "فيض البطمة",
    state_code: "17",
  },
  {
    code: "17029",
    name: "عمورة",
    state_code: "17",
  },
  {
    code: "17030",
    name: "زعفران",
    state_code: "17",
  },
  {
    code: "17031",
    name: "قرنيني",
    state_code: "17",
  },
  {
    code: "17032",
    name: "عين وسارة",
    state_code: "17",
  },
  {
    code: "17033",
    name: "بنهار",
    state_code: "17",
  },
  {
    code: "17034",
    name: "عين معبد",
    state_code: "17",
  },
  {
    code: "17035",
    name: "حاسي بحبح",
    state_code: "17",
  },
  {
    code: "17036",
    name: "مجبارة",
    state_code: "17",
  },
  {
    code: "18001",
    name: "جيجل",
    state_code: "18",
  },
  {
    code: "18002",
    name: "العوانة",
    state_code: "18",
  },
  {
    code: "18003",
    name: "سلمى بن زيادة",
    state_code: "18",
  },
  {
    code: "18004",
    name: "أراقن سويسي",
    state_code: "18",
  },
  {
    code: "18005",
    name: "بوسيف أولاد عسكر",
    state_code: "18",
  },
  {
    code: "18006",
    name: "زيامة منصورية",
    state_code: "18",
  },
  {
    code: "18007",
    name: "الشحنة",
    state_code: "18",
  },
  {
    code: "18008",
    name: "الامير عبد القادر",
    state_code: "18",
  },
  {
    code: "18009",
    name: "وجانة",
    state_code: "18",
  },
  {
    code: "18010",
    name: "الطاهير",
    state_code: "18",
  },
  {
    code: "18011",
    name: "الشقفة",
    state_code: "18",
  },
  {
    code: "18012",
    name: "القنار نشفي",
    state_code: "18",
  },
  {
    code: "18013",
    name: "سيدي عبد العزيز",
    state_code: "18",
  },
  {
    code: "18014",
    name: "الميلية",
    state_code: "18",
  },
  {
    code: "18015",
    name: "أولاد يحيى خدروش",
    state_code: "18",
  },
  {
    code: "18016",
    name: "أولاد رابح",
    state_code: "18",
  },
  {
    code: "18017",
    name: "سيدي معروف",
    state_code: "18",
  },
  {
    code: "18018",
    name: "غبالة",
    state_code: "18",
  },
  {
    code: "18019",
    name: "السطارة",
    state_code: "18",
  },
  {
    code: "18020",
    name: "بوراوي بلهادف",
    state_code: "18",
  },
  {
    code: "18021",
    name: "العنصر",
    state_code: "18",
  },
  {
    code: "18022",
    name: "خيري واد عجول",
    state_code: "18",
  },
  {
    code: "18023",
    name: "جيملة",
    state_code: "18",
  },
  {
    code: "18024",
    name: "قاوس",
    state_code: "18",
  },
  {
    code: "18025",
    name: "تاكسنة",
    state_code: "18",
  },
  {
    code: "18026",
    name: "برج الطهر",
    state_code: "18",
  },
  {
    code: "18027",
    name: "بودريعة بني  ياجيس",
    state_code: "18",
  },
  {
    code: "18028",
    name: "الجمعة بني حبيبي",
    state_code: "18",
  },
  {
    code: "19001",
    name: "الرصفة",
    state_code: "19",
  },
  {
    code: "19002",
    name: "واد البارد",
    state_code: "19",
  },
  {
    code: "19003",
    name: "تيزي نبشار",
    state_code: "19",
  },
  {
    code: "19004",
    name: "مزلوق",
    state_code: "19",
  },
  {
    code: "19005",
    name: "قلال",
    state_code: "19",
  },
  {
    code: "19006",
    name: "قصر الابطال",
    state_code: "19",
  },
  {
    code: "19007",
    name: "أولاد سي أحمد",
    state_code: "19",
  },
  {
    code: "19008",
    name: "أيت نوال مزادة",
    state_code: "19",
  },
  {
    code: "19009",
    name: "ايت تيزي",
    state_code: "19",
  },
  {
    code: "19010",
    name: "بوعنداس",
    state_code: "19",
  },
  {
    code: "19011",
    name: "بوسلام",
    state_code: "19",
  },
  {
    code: "19012",
    name: "حمام السخنة",
    state_code: "19",
  },
  {
    code: "19013",
    name: "الطاية",
    state_code: "19",
  },
  {
    code: "19014",
    name: "التلة",
    state_code: "19",
  },
  {
    code: "19015",
    name: "عين ولمان",
    state_code: "19",
  },
  {
    code: "19016",
    name: "بوطالب",
    state_code: "19",
  },
  {
    code: "19017",
    name: "الحامة",
    state_code: "19",
  },
  {
    code: "19018",
    name: "أولاد تبان",
    state_code: "19",
  },
  {
    code: "19019",
    name: "عموشة",
    state_code: "19",
  },
  {
    code: "19020",
    name: "صالح باي",
    state_code: "19",
  },
  {
    code: "19021",
    name: "عين أزال",
    state_code: "19",
  },
  {
    code: "19022",
    name: "عين الحجر",
    state_code: "19",
  },
  {
    code: "19023",
    name: "بيضاء برج",
    state_code: "19",
  },
  {
    code: "19024",
    name: "بئر حدادة",
    state_code: "19",
  },
  {
    code: "19025",
    name: "قنزات",
    state_code: "19",
  },
  {
    code: "19026",
    name: "حربيل",
    state_code: "19",
  },
  {
    code: "19027",
    name: "عين الروى",
    state_code: "19",
  },
  {
    code: "19028",
    name: "بني وسين",
    state_code: "19",
  },
  {
    code: "19029",
    name: "أوريسيا",
    state_code: "19",
  },
  {
    code: "19030",
    name: "بوقاعة",
    state_code: "19",
  },
  {
    code: "19031",
    name: "ذراع قبيلة",
    state_code: "19",
  },
  {
    code: "19032",
    name: "حمام قرقور",
    state_code: "19",
  },
  {
    code: "19033",
    name: "سطيف",
    state_code: "19",
  },
  {
    code: "19034",
    name: "عين الكبيرة",
    state_code: "19",
  },
  {
    code: "19035",
    name: "الدهامشة",
    state_code: "19",
  },
  {
    code: "19036",
    name: "أولاد عدوان",
    state_code: "19",
  },
  {
    code: "19037",
    name: "عين السبت",
    state_code: "19",
  },
  {
    code: "19038",
    name: "بني عزيز",
    state_code: "19",
  },
  {
    code: "19039",
    name: "معاوية",
    state_code: "19",
  },
  {
    code: "19040",
    name: "بلاعة",
    state_code: "19",
  },
  {
    code: "19041",
    name: "بئر العرش",
    state_code: "19",
  },
  {
    code: "19042",
    name: "الولجة",
    state_code: "19",
  },
  {
    code: "19043",
    name: "تاشودة",
    state_code: "19",
  },
  {
    code: "19044",
    name: "تالة إيفاسن",
    state_code: "19",
  },
  {
    code: "19045",
    name: "سرج الغول",
    state_code: "19",
  },
  {
    code: "19046",
    name: "قجال",
    state_code: "19",
  },
  {
    code: "19047",
    name: "أولاد صابر",
    state_code: "19",
  },
  {
    code: "19048",
    name: "بازر سكرة",
    state_code: "19",
  },
  {
    code: "19049",
    name: "العلمة",
    state_code: "19",
  },
  {
    code: "19050",
    name: "قلتة زرقاء",
    state_code: "19",
  },
  {
    code: "19051",
    name: "بني فودة",
    state_code: "19",
  },
  {
    code: "19052",
    name: "جميلة",
    state_code: "19",
  },
  {
    code: "19053",
    name: "عين لقراج",
    state_code: "19",
  },
  {
    code: "19054",
    name: "بني شبانة",
    state_code: "19",
  },
  {
    code: "19055",
    name: "بني ورتيلان",
    state_code: "19",
  },
  {
    code: "19056",
    name: "بني موحلي",
    state_code: "19",
  },
  {
    code: "19057",
    name: "عين عباسة",
    state_code: "19",
  },
  {
    code: "19058",
    name: "عين أرنات",
    state_code: "19",
  },
  {
    code: "19059",
    name: "بابور",
    state_code: "19",
  },
  {
    code: "19060",
    name: "ماوكلان",
    state_code: "19",
  },
  {
    code: "20001",
    name: "سعيدة",
    state_code: "20",
  },
  {
    code: "20002",
    name: "تيرسين",
    state_code: "20",
  },
  {
    code: "20003",
    name: "أولاد إبراهيم",
    state_code: "20",
  },
  {
    code: "20004",
    name: "عين السلطان",
    state_code: "20",
  },
  {
    code: "20005",
    name: "المعمورة",
    state_code: "20",
  },
  {
    code: "20006",
    name: "الحساسنة",
    state_code: "20",
  },
  {
    code: "20007",
    name: "عين السخونة",
    state_code: "20",
  },
  {
    code: "20008",
    name: "سيدي بوبكر",
    state_code: "20",
  },
  {
    code: "20009",
    name: "أولاد خالد",
    state_code: "20",
  },
  {
    code: "20010",
    name: "هونت",
    state_code: "20",
  },
  {
    code: "20011",
    name: "يوب",
    state_code: "20",
  },
  {
    code: "20012",
    name: "دوي ثابت",
    state_code: "20",
  },
  {
    code: "20013",
    name: "سيدي احمد",
    state_code: "20",
  },
  {
    code: "20014",
    name: "مولاي العربي",
    state_code: "20",
  },
  {
    code: "20015",
    name: "عين الحجر",
    state_code: "20",
  },
  {
    code: "20016",
    name: "سيدي عمر",
    state_code: "20",
  },
  {
    code: "21001",
    name: "عين بوزيان",
    state_code: "21",
  },
  {
    code: "21002",
    name: "صالح بو الشعور",
    state_code: "21",
  },
  {
    code: "21003",
    name: "الحدائق",
    state_code: "21",
  },
  {
    code: "21004",
    name: "زردازة",
    state_code: "21",
  },
  {
    code: "21005",
    name: "أولاد حبابة",
    state_code: "21",
  },
  {
    code: "21006",
    name: "بني ولبان",
    state_code: "21",
  },
  {
    code: "21007",
    name: "سيدي مزغيش",
    state_code: "21",
  },
  {
    code: "21008",
    name: "بني بشير",
    state_code: "21",
  },
  {
    code: "21009",
    name: "رمضان جمال",
    state_code: "21",
  },
  {
    code: "21010",
    name: "بين الويدان",
    state_code: "21",
  },
  {
    code: "21011",
    name: "مجاز الدشيش",
    state_code: "21",
  },
  {
    code: "21012",
    name: "تمالوس",
    state_code: "21",
  },
  {
    code: "21013",
    name: "عين قشرة",
    state_code: "21",
  },
  {
    code: "21014",
    name: "الولجة بولبلوط",
    state_code: "21",
  },
  {
    code: "21015",
    name: "أم الطوب",
    state_code: "21",
  },
  {
    code: "21016",
    name: "الغدير",
    state_code: "21",
  },
  {
    code: "21017",
    name: "الكركرة",
    state_code: "21",
  },
  {
    code: "21018",
    name: "الحروش",
    state_code: "21",
  },
  {
    code: "21019",
    name: "الزيتونة",
    state_code: "21",
  },
  {
    code: "21020",
    name: "أولاد عطية",
    state_code: "21",
  },
  {
    code: "21021",
    name: "وادي الزهور",
    state_code: "21",
  },
  {
    code: "21022",
    name: "القل",
    state_code: "21",
  },
  {
    code: "21023",
    name: "الشرايع",
    state_code: "21",
  },
  {
    code: "21024",
    name: "بني زيد",
    state_code: "21",
  },
  {
    code: "21025",
    name: "خناق مايو",
    state_code: "21",
  },
  {
    code: "21026",
    name: "المرسى",
    state_code: "21",
  },
  {
    code: "21027",
    name: "بن عزوز",
    state_code: "21",
  },
  {
    code: "21028",
    name: "بكوش لخضر",
    state_code: "21",
  },
  {
    code: "21029",
    name: "السبت",
    state_code: "21",
  },
  {
    code: "21030",
    name: "عين شرشار",
    state_code: "21",
  },
  {
    code: "21031",
    name: "عزابة",
    state_code: "21",
  },
  {
    code: "21032",
    name: "بوشطاطة",
    state_code: "21",
  },
  {
    code: "21033",
    name: "فلفلة",
    state_code: "21",
  },
  {
    code: "21034",
    name: "حمادي كرومة",
    state_code: "21",
  },
  {
    code: "21035",
    name: "سكيكدة",
    state_code: "21",
  },
  {
    code: "21036",
    name: "عين زويت",
    state_code: "21",
  },
  {
    code: "21037",
    name: "جندل سعدي محمد",
    state_code: "21",
  },
  {
    code: "21038",
    name: "قنواع",
    state_code: "21",
  },
  {
    code: "22001",
    name: "سيدي علي بن يوب",
    state_code: "22",
  },
  {
    code: "22002",
    name: "مولاي سليسن",
    state_code: "22",
  },
  {
    code: "22003",
    name: "الحصيبة",
    state_code: "22",
  },
  {
    code: "22004",
    name: "عين تندمين",
    state_code: "22",
  },
  {
    code: "22005",
    name: "تنيرة",
    state_code: "22",
  },
  {
    code: "22006",
    name: "وادي سفيون",
    state_code: "22",
  },
  {
    code: "22007",
    name: "حاسي دحو",
    state_code: "22",
  },
  {
    code: "22008",
    name: "وادي تاوريرة",
    state_code: "22",
  },
  {
    code: "22009",
    name: "بن عشيبة شلية",
    state_code: "22",
  },
  {
    code: "22010",
    name: "سيدي يعقوب",
    state_code: "22",
  },
  {
    code: "22011",
    name: "سيدي لحسن",
    state_code: "22",
  },
  {
    code: "22012",
    name: "سيدي خالد",
    state_code: "22",
  },
  {
    code: "22013",
    name: "طابية",
    state_code: "22",
  },
  {
    code: "22014",
    name: "سيدي ابراهيم",
    state_code: "22",
  },
  {
    code: "22015",
    name: "العمارنة",
    state_code: "22",
  },
  {
    code: "22016",
    name: "بوخنفيس",
    state_code: "22",
  },
  {
    code: "22017",
    name: "حاسي زهانة",
    state_code: "22",
  },
  {
    code: "22018",
    name: "شيطوان البلايلة",
    state_code: "22",
  },
  {
    code: "22019",
    name: "بن باديس",
    state_code: "22",
  },
  {
    code: "22020",
    name: "بضرابين المقراني",
    state_code: "22",
  },
  {
    code: "22021",
    name: "سفيزف",
    state_code: "22",
  },
  {
    code: "22022",
    name: "مسيد",
    state_code: "22",
  },
  {
    code: "22023",
    name: "بوجبهة البرج",
    state_code: "22",
  },
  {
    code: "22024",
    name: "عين أدن",
    state_code: "22",
  },
  {
    code: "22025",
    name: "سيدي حمادوش",
    state_code: "22",
  },
  {
    code: "22026",
    name: "سيدي شعيب",
    state_code: "22",
  },
  {
    code: "22027",
    name: "مكدرة",
    state_code: "22",
  },
  {
    code: "22028",
    name: "عين البرد",
    state_code: "22",
  },
  {
    code: "22029",
    name: "رجم دموش",
    state_code: "22",
  },
  {
    code: "22030",
    name: "راس الماء",
    state_code: "22",
  },
  {
    code: "22031",
    name: "وادي السبع",
    state_code: "22",
  },
  {
    code: "22032",
    name: "مرحوم",
    state_code: "22",
  },
  {
    code: "22033",
    name: "سيدي بلعباس",
    state_code: "22",
  },
  {
    code: "22034",
    name: "عين الثريد",
    state_code: "22",
  },
  {
    code: "22035",
    name: "السهالة الثورة",
    state_code: "22",
  },
  {
    code: "22036",
    name: "تسالة",
    state_code: "22",
  },
  {
    code: "22037",
    name: "بلعربي",
    state_code: "22",
  },
  {
    code: "22038",
    name: "مصطفى بن ابراهيم",
    state_code: "22",
  },
  {
    code: "22039",
    name: "تلموني",
    state_code: "22",
  },
  {
    code: "22040",
    name: "زروالة",
    state_code: "22",
  },
  {
    code: "22041",
    name: "الضاية",
    state_code: "22",
  },
  {
    code: "22042",
    name: "مزاورو",
    state_code: "22",
  },
  {
    code: "22043",
    name: "تغاليمت",
    state_code: "22",
  },
  {
    code: "22044",
    name: "تلاغ",
    state_code: "22",
  },
  {
    code: "22045",
    name: "عين قادة",
    state_code: "22",
  },
  {
    code: "22046",
    name: "لمطار",
    state_code: "22",
  },
  {
    code: "22047",
    name: "سيدي علي بوسيدي",
    state_code: "22",
  },
  {
    code: "22048",
    name: "سيدي دحو الزاير",
    state_code: "22",
  },
  {
    code: "22049",
    name: "بئر الحمام",
    state_code: "22",
  },
  {
    code: "22050",
    name: "مرين",
    state_code: "22",
  },
  {
    code: "22051",
    name: "تفسور",
    state_code: "22",
  },
  {
    code: "22052",
    name: "تاودموت",
    state_code: "22",
  },
  {
    code: "23001",
    name: "عنابة",
    state_code: "23",
  },
  {
    code: "23002",
    name: "سرايدي",
    state_code: "23",
  },
  {
    code: "23003",
    name: "برحال",
    state_code: "23",
  },
  {
    code: "23004",
    name: "واد العنب",
    state_code: "23",
  },
  {
    code: "23005",
    name: "الحجار",
    state_code: "23",
  },
  {
    code: "23006",
    name: "سيدي عمار",
    state_code: "23",
  },
  {
    code: "23007",
    name: "البوني",
    state_code: "23",
  },
  {
    code: "23008",
    name: "عين الباردة",
    state_code: "23",
  },
  {
    code: "23009",
    name: "الشرفة",
    state_code: "23",
  },
  {
    code: "23010",
    name: "العلمة",
    state_code: "23",
  },
  {
    code: "23011",
    name: "التريعات",
    state_code: "23",
  },
  {
    code: "23012",
    name: "شطايبي",
    state_code: "23",
  },
  {
    code: "24001",
    name: "نشماية",
    state_code: "24",
  },
  {
    code: "24002",
    name: "بوحمدان",
    state_code: "24",
  },
  {
    code: "24003",
    name: "حمام دباغ",
    state_code: "24",
  },
  {
    code: "24004",
    name: "الركنية",
    state_code: "24",
  },
  {
    code: "24005",
    name: "الدهوارة",
    state_code: "24",
  },
  {
    code: "24006",
    name: "حمام النبايل",
    state_code: "24",
  },
  {
    code: "24007",
    name: "قالمة",
    state_code: "24",
  },
  {
    code: "24008",
    name: "بومهرة أحمد",
    state_code: "24",
  },
  {
    code: "24009",
    name: "عين بن بيضاء",
    state_code: "24",
  },
  {
    code: "24010",
    name: "بوشقوف",
    state_code: "24",
  },
  {
    code: "24011",
    name: "مجاز الصفاء",
    state_code: "24",
  },
  {
    code: "24012",
    name: "وادي فراغة",
    state_code: "24",
  },
  {
    code: "24013",
    name: "بوعاتي محمود",
    state_code: "24",
  },
  {
    code: "24014",
    name: "الفجوج",
    state_code: "24",
  },
  {
    code: "24015",
    name: "هيليوبوليس",
    state_code: "24",
  },
  {
    code: "24016",
    name: "مجاز عمار",
    state_code: "24",
  },
  {
    code: "24017",
    name: "هواري بومدين",
    state_code: "24",
  },
  {
    code: "24018",
    name: "رأس العقبة",
    state_code: "24",
  },
  {
    code: "24019",
    name: "سلاوة عنونة",
    state_code: "24",
  },
  {
    code: "24020",
    name: "جبالة الخميسي",
    state_code: "24",
  },
  {
    code: "24021",
    name: "برج صباط",
    state_code: "24",
  },
  {
    code: "24022",
    name: "وادي الزناتي",
    state_code: "24",
  },
  {
    code: "24023",
    name: "عين رقادة",
    state_code: "24",
  },
  {
    code: "24024",
    name: "عين العربي",
    state_code: "24",
  },
  {
    code: "24025",
    name: "عين مخلوف",
    state_code: "24",
  },
  {
    code: "24026",
    name: "تاملوكة",
    state_code: "24",
  },
  {
    code: "24027",
    name: "عين صندل",
    state_code: "24",
  },
  {
    code: "24028",
    name: "بوحشانة",
    state_code: "24",
  },
  {
    code: "24029",
    name: "لخزارة",
    state_code: "24",
  },
  {
    code: "24030",
    name: "بلخير",
    state_code: "24",
  },
  {
    code: "24031",
    name: "بني مزلين",
    state_code: "24",
  },
  {
    code: "24032",
    name: "قلعة بوصبع",
    state_code: "24",
  },
  {
    code: "24033",
    name: "وادي الشحم",
    state_code: "24",
  },
  {
    code: "24034",
    name: "بن جراح",
    state_code: "24",
  },
  {
    code: "25001",
    name: "ديدوش مراد",
    state_code: "25",
  },
  {
    code: "25002",
    name: "حامة بوزيان",
    state_code: "25",
  },
  {
    code: "25003",
    name: "بني حميدان",
    state_code: "25",
  },
  {
    code: "25004",
    name: "زيغود يوسف",
    state_code: "25",
  },
  {
    code: "25005",
    name: "عين السمارة",
    state_code: "25",
  },
  {
    code: "25006",
    name: "الخروب",
    state_code: "25",
  },
  {
    code: "25007",
    name: "أولاد رحمون",
    state_code: "25",
  },
  {
    code: "25008",
    name: "عين عبيد",
    state_code: "25",
  },
  {
    code: "25009",
    name: "أبن باديس الهرية",
    state_code: "25",
  },
  {
    code: "25010",
    name: "ابن زياد",
    state_code: "25",
  },
  {
    code: "25011",
    name: "بوجريو مسعود",
    state_code: "25",
  },
  {
    code: "25012",
    name: "قسنطينة",
    state_code: "25",
  },
  {
    code: "26001",
    name: "أولاد هلال",
    state_code: "26",
  },
  {
    code: "26002",
    name: "السواقي",
    state_code: "26",
  },
  {
    code: "26003",
    name: "قصر البخاري",
    state_code: "26",
  },
  {
    code: "26004",
    name: "مفاتحة",
    state_code: "26",
  },
  {
    code: "26005",
    name: "السانق",
    state_code: "26",
  },
  {
    code: "26006",
    name: "العزيزية",
    state_code: "26",
  },
  {
    code: "26007",
    name: "مغراوة",
    state_code: "26",
  },
  {
    code: "26008",
    name: "ميهوب",
    state_code: "26",
  },
  {
    code: "26009",
    name: "بوعيش",
    state_code: "26",
  },
  {
    code: "26010",
    name: "بوغزول",
    state_code: "26",
  },
  {
    code: "26011",
    name: "الشهبونية",
    state_code: "26",
  },
  {
    code: "26012",
    name: "حناشة",
    state_code: "26",
  },
  {
    code: "26013",
    name: "عوامري",
    state_code: "26",
  },
  {
    code: "26014",
    name: "وادي حربيل",
    state_code: "26",
  },
  {
    code: "26015",
    name: "بني سليمان",
    state_code: "26",
  },
  {
    code: "26016",
    name: "بوعيشون",
    state_code: "26",
  },
  {
    code: "26017",
    name: "أولاد بوعشرة",
    state_code: "26",
  },
  {
    code: "26018",
    name: "سي المحجوب",
    state_code: "26",
  },
  {
    code: "26019",
    name: "بوسكن",
    state_code: "26",
  },
  {
    code: "26020",
    name: "سيدي الربيع",
    state_code: "26",
  },
  {
    code: "26021",
    name: "البرواقية",
    state_code: "26",
  },
  {
    code: "26022",
    name: "أولاد دايد",
    state_code: "26",
  },
  {
    code: "26023",
    name: "الربعية",
    state_code: "26",
  },
  {
    code: "26024",
    name: "مجبر",
    state_code: "26",
  },
  {
    code: "26025",
    name: "ثلاث دوائر",
    state_code: "26",
  },
  {
    code: "26026",
    name: "الزبيرية",
    state_code: "26",
  },
  {
    code: "26027",
    name: "العيساوية",
    state_code: "26",
  },
  {
    code: "26028",
    name: "الحوضان",
    state_code: "26",
  },
  {
    code: "26029",
    name: "مزغنة",
    state_code: "26",
  },
  {
    code: "26030",
    name: "تابلاط",
    state_code: "26",
  },
  {
    code: "26031",
    name: "بوغار",
    state_code: "26",
  },
  {
    code: "26032",
    name: "سغوان",
    state_code: "26",
  },
  {
    code: "26033",
    name: "ذراع السمار",
    state_code: "26",
  },
  {
    code: "26034",
    name: "المدية",
    state_code: "26",
  },
  {
    code: "26035",
    name: "تمسقيدة",
    state_code: "26",
  },
  {
    code: "26036",
    name: "بن شكاو",
    state_code: "26",
  },
  {
    code: "26037",
    name: "الحمدانية",
    state_code: "26",
  },
  {
    code: "26038",
    name: "وزرة",
    state_code: "26",
  },
  {
    code: "26039",
    name: "تيزي مهدي",
    state_code: "26",
  },
  {
    code: "26040",
    name: "عين بوسيف",
    state_code: "26",
  },
  {
    code: "26041",
    name: "العوينات",
    state_code: "26",
  },
  {
    code: "26042",
    name: "الكاف الاخضر",
    state_code: "26",
  },
  {
    code: "26043",
    name: "أولاد امعرف",
    state_code: "26",
  },
  {
    code: "26044",
    name: "سيدي دامد",
    state_code: "26",
  },
  {
    code: "26045",
    name: "بعطة",
    state_code: "26",
  },
  {
    code: "26046",
    name: "العمارية",
    state_code: "26",
  },
  {
    code: "26047",
    name: "أولاد إبراهيم",
    state_code: "26",
  },
  {
    code: "26048",
    name: "بئر بن عابد",
    state_code: "26",
  },
  {
    code: "26049",
    name: "القلب الكبير",
    state_code: "26",
  },
  {
    code: "26050",
    name: "سدراية",
    state_code: "26",
  },
  {
    code: "26051",
    name: "عين اقصير",
    state_code: "26",
  },
  {
    code: "26052",
    name: "شلالة العذاورة",
    state_code: "26",
  },
  {
    code: "26053",
    name: "شنيقل",
    state_code: "26",
  },
  {
    code: "26054",
    name: "تفراوت",
    state_code: "26",
  },
  {
    code: "26055",
    name: "بوشراحيل",
    state_code: "26",
  },
  {
    code: "26056",
    name: "خمس جوامع",
    state_code: "26",
  },
  {
    code: "26057",
    name: "سيدي نعمان",
    state_code: "26",
  },
  {
    code: "26058",
    name: "عزيز",
    state_code: "26",
  },
  {
    code: "26059",
    name: "دراق",
    state_code: "26",
  },
  {
    code: "26060",
    name: "أم الجليل",
    state_code: "26",
  },
  {
    code: "26061",
    name: "جواب",
    state_code: "26",
  },
  {
    code: "26062",
    name: "سيدي زهار",
    state_code: "26",
  },
  {
    code: "26063",
    name: "سيدي زيان",
    state_code: "26",
  },
  {
    code: "26064",
    name: "أولاد عنتر",
    state_code: "26",
  },
  {
    code: "27001",
    name: "فرناقة",
    state_code: "27",
  },
  {
    code: "27002",
    name: "وادي الخير",
    state_code: "27",
  },
  {
    code: "27003",
    name: "الحسيان (بني ياحي",
    state_code: "27",
  },
  {
    code: "27004",
    name: "حاسي ماماش",
    state_code: "27",
  },
  {
    code: "27005",
    name: "مزغران",
    state_code: "27",
  },
  {
    code: "27006",
    name: "ستيدية",
    state_code: "27",
  },
  {
    code: "27007",
    name: "عين تادلس",
    state_code: "27",
  },
  {
    code: "27008",
    name: "سيدي بلعطار",
    state_code: "27",
  },
  {
    code: "27009",
    name: "سور",
    state_code: "27",
  },
  {
    code: "27010",
    name: "عين بودينار",
    state_code: "27",
  },
  {
    code: "27011",
    name: "خير الدين",
    state_code: "27",
  },
  {
    code: "27012",
    name: "صيادة",
    state_code: "27",
  },
  {
    code: "27013",
    name: "سيدي علي",
    state_code: "27",
  },
  {
    code: "27014",
    name: "تزقايت",
    state_code: "27",
  },
  {
    code: "27015",
    name: "بن عبد المالك رمضان",
    state_code: "27",
  },
  {
    code: "27016",
    name: "مستغانم",
    state_code: "27",
  },
  {
    code: "27017",
    name: "حجاج",
    state_code: "27",
  },
  {
    code: "27018",
    name: "سيدي لخضر",
    state_code: "27",
  },
  {
    code: "27019",
    name: "عشعاشة",
    state_code: "27",
  },
  {
    code: "27020",
    name: "خضرة",
    state_code: "27",
  },
  {
    code: "27021",
    name: "نكمارية",
    state_code: "27",
  },
  {
    code: "27022",
    name: "أولاد بوغالم",
    state_code: "27",
  },
  {
    code: "27023",
    name: "بوقيراط",
    state_code: "27",
  },
  {
    code: "27024",
    name: "صفصاف",
    state_code: "27",
  },
  {
    code: "27025",
    name: "سيرات",
    state_code: "27",
  },
  {
    code: "27026",
    name: "السوافلية",
    state_code: "27",
  },
  {
    code: "27027",
    name: "عين سيدي الشريف",
    state_code: "27",
  },
  {
    code: "27028",
    name: "منصورة",
    state_code: "27",
  },
  {
    code: "27029",
    name: "ماسرة",
    state_code: "27",
  },
  {
    code: "27030",
    name: "الطواهرية",
    state_code: "27",
  },
  {
    code: "27031",
    name: "عين نويسي",
    state_code: "27",
  },
  {
    code: "27032",
    name: "أولاد مع الله",
    state_code: "27",
  },
  {
    code: "28001",
    name: "شلال",
    state_code: "28",
  },
  {
    code: "28002",
    name: "أولاد ماضي",
    state_code: "28",
  },
  {
    code: "28003",
    name: "خطوطي سد الجير",
    state_code: "28",
  },
  {
    code: "28004",
    name: "بلعايبة",
    state_code: "28",
  },
  {
    code: "28005",
    name: "برهوم",
    state_code: "28",
  },
  {
    code: "28006",
    name: "دهاهنة",
    state_code: "28",
  },
  {
    code: "28007",
    name: "مقرة",
    state_code: "28",
  },
  {
    code: "28008",
    name: "بني يلمان",
    state_code: "28",
  },
  {
    code: "28009",
    name: "بوطي السايح",
    state_code: "28",
  },
  {
    code: "28010",
    name: "سيدي عيسى",
    state_code: "28",
  },
  {
    code: "28011",
    name: "عين الحجل",
    state_code: "28",
  },
  {
    code: "28012",
    name: "سيدي هجرس",
    state_code: "28",
  },
  {
    code: "28013",
    name: "بوسعادة",
    state_code: "28",
  },
  {
    code: "28014",
    name: "الهامل",
    state_code: "28",
  },
  {
    code: "28015",
    name: "ولتام",
    state_code: "28",
  },
  {
    code: "28016",
    name: "بن زوه",
    state_code: "28",
  },
  {
    code: "28017",
    name: "أولاد سيدي ابراهيم",
    state_code: "28",
  },
  {
    code: "28018",
    name: "سيدي عامر",
    state_code: "28",
  },
  {
    code: "28019",
    name: "تامسة",
    state_code: "28",
  },
  {
    code: "28020",
    name: "بن سرور",
    state_code: "28",
  },
  {
    code: "28021",
    name: "محمد بوضياف",
    state_code: "28",
  },
  {
    code: "28022",
    name: "أولاد سليمان",
    state_code: "28",
  },
  {
    code: "28023",
    name: "زرزور",
    state_code: "28",
  },
  {
    code: "28024",
    name: "عين الملح",
    state_code: "28",
  },
  {
    code: "28025",
    name: "عين فارس",
    state_code: "28",
  },
  {
    code: "28026",
    name: "عين الريش",
    state_code: "28",
  },
  {
    code: "28027",
    name: "بئر فضة",
    state_code: "28",
  },
  {
    code: "28028",
    name: "سيدي امحمد",
    state_code: "28",
  },
  {
    code: "28029",
    name: "امجدل",
    state_code: "28",
  },
  {
    code: "28030",
    name: "مناعة",
    state_code: "28",
  },
  {
    code: "28031",
    name: "جبل مساعد",
    state_code: "28",
  },
  {
    code: "28032",
    name: "سليم",
    state_code: "28",
  },
  {
    code: "28033",
    name: "المسيلة",
    state_code: "28",
  },
  {
    code: "28034",
    name: "حمام الضلعة",
    state_code: "28",
  },
  {
    code: "28035",
    name: "ونوغة",
    state_code: "28",
  },
  {
    code: "28036",
    name: "أولاد منصور",
    state_code: "28",
  },
  {
    code: "28037",
    name: "تارمونت",
    state_code: "28",
  },
  {
    code: "28038",
    name: "المعاضيد",
    state_code: "28",
  },
  {
    code: "28039",
    name: "المطارفة",
    state_code: "28",
  },
  {
    code: "28040",
    name: "معاريف",
    state_code: "28",
  },
  {
    code: "28041",
    name: "أولاد دراج",
    state_code: "28",
  },
  {
    code: "28042",
    name: "السوامع",
    state_code: "28",
  },
  {
    code: "28043",
    name: "الحوامد",
    state_code: "28",
  },
  {
    code: "28044",
    name: "خبانة",
    state_code: "28",
  },
  {
    code: "28045",
    name: "مسيف",
    state_code: "28",
  },
  {
    code: "28046",
    name: "عين الخضراء",
    state_code: "28",
  },
  {
    code: "28047",
    name: "أولاد عدي لقبالة",
    state_code: "28",
  },
  {
    code: "29001",
    name: "وادي الأبطال",
    state_code: "29",
  },
  {
    code: "29002",
    name: "سيدي عبد المومن",
    state_code: "29",
  },
  {
    code: "29003",
    name: "سجرارة",
    state_code: "29",
  },
  {
    code: "29004",
    name: "المحمدية",
    state_code: "29",
  },
  {
    code: "29005",
    name: "تيغنيف",
    state_code: "29",
  },
  {
    code: "29006",
    name: "مقطع الدوز",
    state_code: "29",
  },
  {
    code: "29007",
    name: "فراقيق",
    state_code: "29",
  },
  {
    code: "29008",
    name: "الغمري",
    state_code: "29",
  },
  {
    code: "29009",
    name: "زهانة",
    state_code: "29",
  },
  {
    code: "29010",
    name: "القعدة",
    state_code: "29",
  },
  {
    code: "29011",
    name: "رأس عين عميروش",
    state_code: "29",
  },
  {
    code: "29012",
    name: "عقاز",
    state_code: "29",
  },
  {
    code: "29013",
    name: "العلايمية",
    state_code: "29",
  },
  {
    code: "29014",
    name: "سيق",
    state_code: "29",
  },
  {
    code: "29015",
    name: "الشرفاء",
    state_code: "29",
  },
  {
    code: "29016",
    name: "بوهني",
    state_code: "29",
  },
  {
    code: "29017",
    name: "المأمونية",
    state_code: "29",
  },
  {
    code: "29018",
    name: "القطنة",
    state_code: "29",
  },
  {
    code: "29019",
    name: "عين فارس",
    state_code: "29",
  },
  {
    code: "29020",
    name: "غروس",
    state_code: "29",
  },
  {
    code: "29021",
    name: "بنيان",
    state_code: "29",
  },
  {
    code: "29022",
    name: "عوف",
    state_code: "29",
  },
  {
    code: "29023",
    name: "قرجوم",
    state_code: "29",
  },
  {
    code: "29024",
    name: "عين أفرص",
    state_code: "29",
  },
  {
    code: "29025",
    name: "عين فكان",
    state_code: "29",
  },
  {
    code: "29026",
    name: "خلوية",
    state_code: "29",
  },
  {
    code: "29027",
    name: "المنور",
    state_code: "29",
  },
  {
    code: "29028",
    name: "البرج",
    state_code: "29",
  },
  {
    code: "29029",
    name: "سيدي بوسعيد",
    state_code: "29",
  },
  {
    code: "29030",
    name: "المطمور",
    state_code: "29",
  },
  {
    code: "29031",
    name: "سيدي قادة",
    state_code: "29",
  },
  {
    code: "29032",
    name: "ماقضة",
    state_code: "29",
  },
  {
    code: "29033",
    name: "معسكر",
    state_code: "29",
  },
  {
    code: "29034",
    name: "بوحنيفية",
    state_code: "29",
  },
  {
    code: "29035",
    name: "غريس",
    state_code: "29",
  },
  {
    code: "29036",
    name: "حسين",
    state_code: "29",
  },
  {
    code: "29037",
    name: "القرط",
    state_code: "29",
  },
  {
    code: "29038",
    name: "فروحة",
    state_code: "29",
  },
  {
    code: "29039",
    name: "تيزي",
    state_code: "29",
  },
  {
    code: "29040",
    name: "السهايلية",
    state_code: "29",
  },
  {
    code: "29041",
    name: "ماوسة",
    state_code: "29",
  },
  {
    code: "29042",
    name: "سيدي عبد الجبار",
    state_code: "29",
  },
  {
    code: "29043",
    name: "الحشم",
    state_code: "29",
  },
  {
    code: "29044",
    name: "نسمط",
    state_code: "29",
  },
  {
    code: "29045",
    name: "زلامطة",
    state_code: "29",
  },
  {
    code: "29046",
    name: "عين فراح",
    state_code: "29",
  },
  {
    code: "29047",
    name: "وادي التاغية",
    state_code: "29",
  },
  {
    code: "30001",
    name: "ورقلة",
    state_code: "30",
  },
  {
    code: "30002",
    name: "حاسي مسعود",
    state_code: "30",
  },
  {
    code: "30003",
    name: "عين البيضاء",
    state_code: "30",
  },
  {
    code: "30004",
    name: "حاسي بن عبد الله",
    state_code: "30",
  },
  {
    code: "30005",
    name: "سيدي خويلد",
    state_code: "30",
  },
  {
    code: "30006",
    name: "البرمة",
    state_code: "30",
  },
  {
    code: "30007",
    name: "الرويسات",
    state_code: "30",
  },
  {
    code: "30008",
    name: "انقوسة",
    state_code: "30",
  },
  {
    code: "31001",
    name: "سيدي الشحمي",
    state_code: "31",
  },
  {
    code: "31002",
    name: "حاسي مفسوخ",
    state_code: "31",
  },
  {
    code: "31003",
    name: "بئر الجير",
    state_code: "31",
  },
  {
    code: "31004",
    name: "حاسي بن عقبة",
    state_code: "31",
  },
  {
    code: "31005",
    name: "قديل",
    state_code: "31",
  },
  {
    code: "31006",
    name: "حاسي بونيف",
    state_code: "31",
  },
  {
    code: "31007",
    name: "الكرمة",
    state_code: "31",
  },
  {
    code: "31008",
    name: "السانية",
    state_code: "31",
  },
  {
    code: "31009",
    name: "بن فريحة",
    state_code: "31",
  },
  {
    code: "31010",
    name: "أرزيو",
    state_code: "31",
  },
  {
    code: "31011",
    name: "سيدي بن يبقى",
    state_code: "31",
  },
  {
    code: "31012",
    name: "عين البية",
    state_code: "31",
  },
  {
    code: "31013",
    name: "بطيوة",
    state_code: "31",
  },
  {
    code: "31014",
    name: "مرسى الحجاج",
    state_code: "31",
  },
  {
    code: "31015",
    name: "عين الترك",
    state_code: "31",
  },
  {
    code: "31016",
    name: "وهران",
    state_code: "31",
  },
  {
    code: "31017",
    name: "العنصر",
    state_code: "31",
  },
  {
    code: "31018",
    name: "المرسى الكبير",
    state_code: "31",
  },
  {
    code: "31019",
    name: "بوفاتيس",
    state_code: "31",
  },
  {
    code: "31020",
    name: "البراية",
    state_code: "31",
  },
  {
    code: "31021",
    name: "وادي تليلات",
    state_code: "31",
  },
  {
    code: "31022",
    name: "عين الكرمة",
    state_code: "31",
  },
  {
    code: "31023",
    name: "بوتليليس",
    state_code: "31",
  },
  {
    code: "31024",
    name: "مسرغين",
    state_code: "31",
  },
  {
    code: "31025",
    name: "بوسفر",
    state_code: "31",
  },
  {
    code: "31026",
    name: "طفراوي",
    state_code: "31",
  },
  {
    code: "32001",
    name: "عين العراك",
    state_code: "32",
  },
  {
    code: "32002",
    name: "كراكدة",
    state_code: "32",
  },
  {
    code: "32003",
    name: "سيدي سليمان",
    state_code: "32",
  },
  {
    code: "32004",
    name: "سيدي عامر",
    state_code: "32",
  },
  {
    code: "32005",
    name: "بوعلام",
    state_code: "32",
  },
  {
    code: "32006",
    name: "البنود",
    state_code: "32",
  },
  {
    code: "32007",
    name: "بوقطب",
    state_code: "32",
  },
  {
    code: "32008",
    name: "الخيثر",
    state_code: "32",
  },
  {
    code: "32009",
    name: "توسمولين",
    state_code: "32",
  },
  {
    code: "32010",
    name: "سيدي طيفور",
    state_code: "32",
  },
  {
    code: "32011",
    name: "ستيتن",
    state_code: "32",
  },
  {
    code: "32012",
    name: "البيض",
    state_code: "32",
  },
  {
    code: "32013",
    name: "رقاصة",
    state_code: "32",
  },
  {
    code: "32014",
    name: "المحرة",
    state_code: "32",
  },
  {
    code: "32015",
    name: "الكاف الأحمر",
    state_code: "32",
  },
  {
    code: "32016",
    name: "بريزينة",
    state_code: "32",
  },
  {
    code: "32017",
    name: "الغاسول",
    state_code: "32",
  },
  {
    code: "32018",
    name: "الأبيض سيدي الشيخ",
    state_code: "32",
  },
  {
    code: "32019",
    name: "بوسمغون",
    state_code: "32",
  },
  {
    code: "32020",
    name: "الشقيق",
    state_code: "32",
  },
  {
    code: "32021",
    name: "شلالة",
    state_code: "32",
  },
  {
    code: "32022",
    name: "اربوات",
    state_code: "32",
  },
  {
    code: "33001",
    name: "برج عمر إدريس",
    state_code: "33",
  },
  {
    code: "33002",
    name: "دبداب",
    state_code: "33",
  },
  {
    code: "33003",
    name: "إن أمناس",
    state_code: "33",
  },
  {
    code: "33004",
    name: "إيليزي",
    state_code: "33",
  },
  {
    code: "34001",
    name: "الحمادية",
    state_code: "34",
  },
  {
    code: "34002",
    name: "أولاد سيدي ابراهيم",
    state_code: "34",
  },
  {
    code: "34003",
    name: "عين تاغروت",
    state_code: "34",
  },
  {
    code: "34004",
    name: "تيكستار",
    state_code: "34",
  },
  {
    code: "34005",
    name: "بليمور",
    state_code: "34",
  },
  {
    code: "34006",
    name: "العناصر",
    state_code: "34",
  },
  {
    code: "34007",
    name: "غيلاسة",
    state_code: "34",
  },
  {
    code: "34008",
    name: "تقلعيت",
    state_code: "34",
  },
  {
    code: "34009",
    name: "برج الغدير",
    state_code: "34",
  },
  {
    code: "34010",
    name: "العش",
    state_code: "34",
  },
  {
    code: "34011",
    name: "سيدي أمبارك",
    state_code: "34",
  },
  {
    code: "34012",
    name: "خليل",
    state_code: "34",
  },
  {
    code: "34013",
    name: "بئر قاصد علي",
    state_code: "34",
  },
  {
    code: "34014",
    name: "تفرق",
    state_code: "34",
  },
  {
    code: "34015",
    name: "الماين",
    state_code: "34",
  },
  {
    code: "34016",
    name: "جعافرة",
    state_code: "34",
  },
  {
    code: "34017",
    name: "القلة",
    state_code: "34",
  },
  {
    code: "34018",
    name: "ثنية النصر",
    state_code: "34",
  },
  {
    code: "34019",
    name: "المهير",
    state_code: "34",
  },
  {
    code: "34020",
    name: "القصور",
    state_code: "34",
  },
  {
    code: "34021",
    name: "المنصورة",
    state_code: "34",
  },
  {
    code: "34022",
    name: "حرازة",
    state_code: "34",
  },
  {
    code: "34023",
    name: "الرابطة",
    state_code: "34",
  },
  {
    code: "34024",
    name: "الياشير",
    state_code: "34",
  },
  {
    code: "34025",
    name: "حسناوة",
    state_code: "34",
  },
  {
    code: "34026",
    name: "مجانة",
    state_code: "34",
  },
  {
    code: "34027",
    name: "عين تسرة",
    state_code: "34",
  },
  {
    code: "34028",
    name: "أولاد أبراهم",
    state_code: "34",
  },
  {
    code: "34029",
    name: "رأس الوادي",
    state_code: "34",
  },
  {
    code: "34030",
    name: "برج زمورة",
    state_code: "34",
  },
  {
    code: "34031",
    name: "أولاد دحمان",
    state_code: "34",
  },
  {
    code: "34032",
    name: "تسامرت",
    state_code: "34",
  },
  {
    code: "34033",
    name: "برج بوعريرج",
    state_code: "34",
  },
  {
    code: "34034",
    name: "بن داود",
    state_code: "34",
  },
  {
    code: "35001",
    name: "الخروبة",
    state_code: "35",
  },
  {
    code: "35002",
    name: "دلس",
    state_code: "35",
  },
  {
    code: "35003",
    name: "بن شود",
    state_code: "35",
  },
  {
    code: "35004",
    name: "أعفير",
    state_code: "35",
  },
  {
    code: "35005",
    name: "الثنية",
    state_code: "35",
  },
  {
    code: "35006",
    name: "بني عمران",
    state_code: "35",
  },
  {
    code: "35007",
    name: "خميس الخشنة",
    state_code: "35",
  },
  {
    code: "35008",
    name: "عمال",
    state_code: "35",
  },
  {
    code: "35009",
    name: "تيمزريت",
    state_code: "35",
  },
  {
    code: "35010",
    name: "زموري",
    state_code: "35",
  },
  {
    code: "35011",
    name: "الاربعطاش",
    state_code: "35",
  },
  {
    code: "35012",
    name: "يسر",
    state_code: "35",
  },
  {
    code: "35013",
    name: "شعبة العامر",
    state_code: "35",
  },
  {
    code: "35014",
    name: "أولاد عيسى",
    state_code: "35",
  },
  {
    code: "35015",
    name: "الناصرية",
    state_code: "35",
  },
  {
    code: "35016",
    name: "بوزقزة قدارة",
    state_code: "35",
  },
  {
    code: "35017",
    name: "سوق الحد",
    state_code: "35",
  },
  {
    code: "35018",
    name: "سيدي داود",
    state_code: "35",
  },
  {
    code: "35019",
    name: "بغلية",
    state_code: "35",
  },
  {
    code: "35020",
    name: "لقاطة",
    state_code: "35",
  },
  {
    code: "35021",
    name: "جنات",
    state_code: "35",
  },
  {
    code: "35022",
    name: "تيجلابين",
    state_code: "35",
  },
  {
    code: "35023",
    name: "سي مصطفى",
    state_code: "35",
  },
  {
    code: "35024",
    name: "أولاد هداج",
    state_code: "35",
  },
  {
    code: "35025",
    name: "أولاد موسى",
    state_code: "35",
  },
  {
    code: "35026",
    name: "بومرداس",
    state_code: "35",
  },
  {
    code: "35027",
    name: "قورصو",
    state_code: "35",
  },
  {
    code: "35028",
    name: "برج منايل",
    state_code: "35",
  },
  {
    code: "35029",
    name: "بودواو",
    state_code: "35",
  },
  {
    code: "35030",
    name: "بودواو البحري",
    state_code: "35",
  },
  {
    code: "35031",
    name: "تاورقة",
    state_code: "35",
  },
  {
    code: "35032",
    name: "حمادي",
    state_code: "35",
  },
  {
    code: "36001",
    name: "عين العسل",
    state_code: "36",
  },
  {
    code: "36002",
    name: "بوقوس",
    state_code: "36",
  },
  {
    code: "36003",
    name: "الطارف",
    state_code: "36",
  },
  {
    code: "36004",
    name: "الزيتونة",
    state_code: "36",
  },
  {
    code: "36005",
    name: "البسباس",
    state_code: "36",
  },
  {
    code: "36006",
    name: "عين الكرمة",
    state_code: "36",
  },
  {
    code: "36007",
    name: "بوحجار",
    state_code: "36",
  },
  {
    code: "36008",
    name: "حمام بني صالح",
    state_code: "36",
  },
  {
    code: "36009",
    name: "وادي الزيتون",
    state_code: "36",
  },
  {
    code: "36010",
    name: "بن مهيدي",
    state_code: "36",
  },
  {
    code: "36011",
    name: "بريحان",
    state_code: "36",
  },
  {
    code: "36012",
    name: "شبيطة مختار",
    state_code: "36",
  },
  {
    code: "36013",
    name: "الشط",
    state_code: "36",
  },
  {
    code: "36014",
    name: "العيون",
    state_code: "36",
  },
  {
    code: "36015",
    name: "القالة",
    state_code: "36",
  },
  {
    code: "36016",
    name: "السوارخ",
    state_code: "36",
  },
  {
    code: "36017",
    name: "زريزر",
    state_code: "36",
  },
  {
    code: "36018",
    name: "بوثلجة",
    state_code: "36",
  },
  {
    code: "36019",
    name: "الشافية",
    state_code: "36",
  },
  {
    code: "36020",
    name: "بحيرة الطيور",
    state_code: "36",
  },
  {
    code: "36021",
    name: "شحاني",
    state_code: "36",
  },
  {
    code: "36022",
    name: "رمل السوق",
    state_code: "36",
  },
  {
    code: "36023",
    name: "عصفور",
    state_code: "36",
  },
  {
    code: "36024",
    name: "الذرعـان",
    state_code: "36",
  },
  {
    code: "37001",
    name: "تندوف",
    state_code: "37",
  },
  {
    code: "37002",
    name: "أم العسل",
    state_code: "37",
  },
  {
    code: "38001",
    name: "خميستي",
    state_code: "38",
  },
  {
    code: "38002",
    name: "ثنية الاحد",
    state_code: "38",
  },
  {
    code: "38003",
    name: "أولاد بسام",
    state_code: "38",
  },
  {
    code: "38004",
    name: "سيدي بوتوشنت",
    state_code: "38",
  },
  {
    code: "38005",
    name: "تيسمسيلت",
    state_code: "38",
  },
  {
    code: "38006",
    name: "سيدي العنتري",
    state_code: "38",
  },
  {
    code: "38007",
    name: "بني شعيب",
    state_code: "38",
  },
  {
    code: "38008",
    name: "بني لحسن",
    state_code: "38",
  },
  {
    code: "38009",
    name: "سيدي عابد",
    state_code: "38",
  },
  {
    code: "38010",
    name: "سيدي سليمان",
    state_code: "38",
  },
  {
    code: "38011",
    name: "بوقائد",
    state_code: "38",
  },
  {
    code: "38012",
    name: "الأربعاء",
    state_code: "38",
  },
  {
    code: "38013",
    name: "الأزهرية",
    state_code: "38",
  },
  {
    code: "38014",
    name: "لرجام",
    state_code: "38",
  },
  {
    code: "38015",
    name: "الملعب",
    state_code: "38",
  },
  {
    code: "38016",
    name: "العيون",
    state_code: "38",
  },
  {
    code: "38017",
    name: "تملاحت",
    state_code: "38",
  },
  {
    code: "38018",
    name: "اليوسفية",
    state_code: "38",
  },
  {
    code: "38019",
    name: "برج الأمير عبد القادر",
    state_code: "38",
  },
  {
    code: "38020",
    name: "عماري",
    state_code: "38",
  },
  {
    code: "38021",
    name: "المعاصم",
    state_code: "38",
  },
  {
    code: "38022",
    name: "برج بونعامة",
    state_code: "38",
  },
  {
    code: "39001",
    name: "دوار الماء",
    state_code: "39",
  },
  {
    code: "39002",
    name: "العقلة",
    state_code: "39",
  },
  {
    code: "39003",
    name: "المقرن",
    state_code: "39",
  },
  {
    code: "39004",
    name: "سيدي عون",
    state_code: "39",
  },
  {
    code: "39005",
    name: "اميه وانسة",
    state_code: "39",
  },
  {
    code: "39006",
    name: "كوينين",
    state_code: "39",
  },
  {
    code: "39007",
    name: "البياضة",
    state_code: "39",
  },
  {
    code: "39008",
    name: "النخلة",
    state_code: "39",
  },
  {
    code: "39009",
    name: "الرباح",
    state_code: "39",
  },
  {
    code: "39010",
    name: "قمار",
    state_code: "39",
  },
  {
    code: "39011",
    name: "بن  قشة",
    state_code: "39",
  },
  {
    code: "39012",
    name: "ورماس",
    state_code: "39",
  },
  {
    code: "39013",
    name: "تغزوت",
    state_code: "39",
  },
  {
    code: "39014",
    name: "الحمراية",
    state_code: "39",
  },
  {
    code: "39015",
    name: "الرقيبة",
    state_code: "39",
  },
  {
    code: "39016",
    name: "الدبيلة",
    state_code: "39",
  },
  {
    code: "39017",
    name: "حساني عبد الكريم",
    state_code: "39",
  },
  {
    code: "39018",
    name: "حاسي خليفة",
    state_code: "39",
  },
  {
    code: "39019",
    name: "الطريفاوي",
    state_code: "39",
  },
  {
    code: "39020",
    name: "الطالب العربي",
    state_code: "39",
  },
  {
    code: "39021",
    name: "وادي العلندة",
    state_code: "39",
  },
  {
    code: "39022",
    name: "الوادي",
    state_code: "39",
  },
  {
    code: "40001",
    name: "خيران",
    state_code: "40",
  },
  {
    code: "40002",
    name: "بابار",
    state_code: "40",
  },
  {
    code: "40003",
    name: "المحمل",
    state_code: "40",
  },
  {
    code: "40004",
    name: "أولاد رشاش",
    state_code: "40",
  },
  {
    code: "40005",
    name: "جلال",
    state_code: "40",
  },
  {
    code: "40006",
    name: "يابوس",
    state_code: "40",
  },
  {
    code: "40007",
    name: "خنشلة",
    state_code: "40",
  },
  {
    code: "40008",
    name: "قايس",
    state_code: "40",
  },
  {
    code: "40009",
    name: "شلية",
    state_code: "40",
  },
  {
    code: "40010",
    name: "الرميلة",
    state_code: "40",
  },
  {
    code: "40011",
    name: "تاوزيانت",
    state_code: "40",
  },
  {
    code: "40012",
    name: "بغاي",
    state_code: "40",
  },
  {
    code: "40013",
    name: "الحامة",
    state_code: "40",
  },
  {
    code: "40014",
    name: "انسيغة",
    state_code: "40",
  },
  {
    code: "40015",
    name: "طامزة",
    state_code: "40",
  },
  {
    code: "40016",
    name: "عين الطويلة",
    state_code: "40",
  },
  {
    code: "40017",
    name: "متوسة",
    state_code: "40",
  },
  {
    code: "40018",
    name: "بوحمامة",
    state_code: "40",
  },
  {
    code: "40019",
    name: "الولجة",
    state_code: "40",
  },
  {
    code: "40020",
    name: "مصارة",
    state_code: "40",
  },
  {
    code: "40021",
    name: "ششار",
    state_code: "40",
  },
  {
    code: "41001",
    name: "سوق أهراس",
    state_code: "41",
  },
  {
    code: "41002",
    name: "عين سلطان",
    state_code: "41",
  },
  {
    code: "41003",
    name: "سدراتة",
    state_code: "41",
  },
  {
    code: "41004",
    name: "الحنانشة",
    state_code: "41",
  },
  {
    code: "41005",
    name: "المشروحة",
    state_code: "41",
  },
  {
    code: "41006",
    name: "عين الزانة",
    state_code: "41",
  },
  {
    code: "41007",
    name: "أولاد إدريس",
    state_code: "41",
  },
  {
    code: "41008",
    name: "ترقالت",
    state_code: "41",
  },
  {
    code: "41009",
    name: "أم العظايم",
    state_code: "41",
  },
  {
    code: "41010",
    name: "وادي الكبريت",
    state_code: "41",
  },
  {
    code: "41011",
    name: "تيفاش",
    state_code: "41",
  },
  {
    code: "41012",
    name: "الراقوبة",
    state_code: "41",
  },
  {
    code: "41013",
    name: "الدريعة",
    state_code: "41",
  },
  {
    code: "41014",
    name: "تاورة",
    state_code: "41",
  },
  {
    code: "41015",
    name: "الزعرورية",
    state_code: "41",
  },
  {
    code: "41016",
    name: "الحدادة",
    state_code: "41",
  },
  {
    code: "41017",
    name: "الخضارة",
    state_code: "41",
  },
  {
    code: "41018",
    name: "أولاد مومن",
    state_code: "41",
  },
  {
    code: "41019",
    name: "المراهنة",
    state_code: "41",
  },
  {
    code: "41020",
    name: "ويلان",
    state_code: "41",
  },
  {
    code: "41021",
    name: "سيدي فرج",
    state_code: "41",
  },
  {
    code: "41022",
    name: "بئر بوحوش",
    state_code: "41",
  },
  {
    code: "41023",
    name: "سافل الويدان",
    state_code: "41",
  },
  {
    code: "41024",
    name: "خميسة",
    state_code: "41",
  },
  {
    code: "41025",
    name: "مداوروش",
    state_code: "41",
  },
  {
    code: "41026",
    name: "الزوابي",
    state_code: "41",
  },
  {
    code: "42001",
    name: "حجوط",
    state_code: "42",
  },
  {
    code: "42002",
    name: "مراد",
    state_code: "42",
  },
  {
    code: "42003",
    name: "مناصر",
    state_code: "42",
  },
  {
    code: "42004",
    name: "أغبال",
    state_code: "42",
  },
  {
    code: "42005",
    name: "الناظور",
    state_code: "42",
  },
  {
    code: "42006",
    name: "سيدي عامر",
    state_code: "42",
  },
  {
    code: "42007",
    name: "قوراية",
    state_code: "42",
  },
  {
    code: "42008",
    name: "مسلمون",
    state_code: "42",
  },
  {
    code: "42009",
    name: "شرشال",
    state_code: "42",
  },
  {
    code: "42010",
    name: "حجرة النص",
    state_code: "42",
  },
  {
    code: "42011",
    name: "سيدي غيلاس",
    state_code: "42",
  },
  {
    code: "42012",
    name: "الداموس",
    state_code: "42",
  },
  {
    code: "42013",
    name: "الأرهاط",
    state_code: "42",
  },
  {
    code: "42014",
    name: "فوكة",
    state_code: "42",
  },
  {
    code: "42015",
    name: "عين تاقورايت",
    state_code: "42",
  },
  {
    code: "42016",
    name: "بوهارون",
    state_code: "42",
  },
  {
    code: "42017",
    name: "بواسماعيل",
    state_code: "42",
  },
  {
    code: "42018",
    name: "خميستي",
    state_code: "42",
  },
  {
    code: "42019",
    name: "أحمر العين",
    state_code: "42",
  },
  {
    code: "42020",
    name: "بورقيقة",
    state_code: "42",
  },
  {
    code: "42021",
    name: "دواودة",
    state_code: "42",
  },
  {
    code: "42022",
    name: "سيدي راشد",
    state_code: "42",
  },
  {
    code: "42023",
    name: "الحطاطبة",
    state_code: "42",
  },
  {
    code: "42024",
    name: "الشعيبة",
    state_code: "42",
  },
  {
    code: "42025",
    name: "القليعة",
    state_code: "42",
  },
  {
    code: "42026",
    name: "سيدي سميان",
    state_code: "42",
  },
  {
    code: "42027",
    name: "تيبازة",
    state_code: "42",
  },
  {
    code: "42028",
    name: "بني ميلك",
    state_code: "42",
  },
  {
    code: "43001",
    name: "مشيرة",
    state_code: "43",
  },
  {
    code: "43002",
    name: "العياضي برباس",
    state_code: "43",
  },
  {
    code: "43003",
    name: "عين البيضاء أحريش",
    state_code: "43",
  },
  {
    code: "43004",
    name: "تسالة لمطاعي",
    state_code: "43",
  },
  {
    code: "43005",
    name: "ترعي باينان",
    state_code: "43",
  },
  {
    code: "43006",
    name: "اعميرة اراس",
    state_code: "43",
  },
  {
    code: "43007",
    name: "تسدان حدادة",
    state_code: "43",
  },
  {
    code: "43008",
    name: "مينار زارزة",
    state_code: "43",
  },
  {
    code: "43009",
    name: "سيدي مروان",
    state_code: "43",
  },
  {
    code: "43010",
    name: "الشيقارة",
    state_code: "43",
  },
  {
    code: "43011",
    name: "حمالة",
    state_code: "43",
  },
  {
    code: "43012",
    name: "القرارم قوقة",
    state_code: "43",
  },
  {
    code: "43013",
    name: "تيبرقنت",
    state_code: "43",
  },
  {
    code: "43014",
    name: "الرواشد",
    state_code: "43",
  },
  {
    code: "43015",
    name: "دراحي بوصلاح",
    state_code: "43",
  },
  {
    code: "43016",
    name: "زغاية",
    state_code: "43",
  },
  {
    code: "43017",
    name: "وادي النجاء",
    state_code: "43",
  },
  {
    code: "43018",
    name: "أحمد راشدي",
    state_code: "43",
  },
  {
    code: "43019",
    name: "تاجنانت",
    state_code: "43",
  },
  {
    code: "43020",
    name: "عين الملوك",
    state_code: "43",
  },
  {
    code: "43021",
    name: "أولاد اخلوف",
    state_code: "43",
  },
  {
    code: "43022",
    name: "بن يحي عبد الرحمن",
    state_code: "43",
  },
  {
    code: "43023",
    name: "التلاغمة",
    state_code: "43",
  },
  {
    code: "43024",
    name: "وادي سقان",
    state_code: "43",
  },
  {
    code: "43025",
    name: "وادي العثمانية",
    state_code: "43",
  },
  {
    code: "43026",
    name: "عين التين",
    state_code: "43",
  },
  {
    code: "43027",
    name: "شلغوم العيد",
    state_code: "43",
  },
  {
    code: "43028",
    name: "يحي بني قشة",
    state_code: "43",
  },
  {
    code: "43029",
    name: "فرجيوة",
    state_code: "43",
  },
  {
    code: "43030",
    name: "سيدي خليفة",
    state_code: "43",
  },
  {
    code: "43031",
    name: "ميلة",
    state_code: "43",
  },
  {
    code: "43032",
    name: "بوحاتم",
    state_code: "43",
  },
  {
    code: "44001",
    name: "خميس مليانة",
    state_code: "44",
  },
  {
    code: "44002",
    name: "سيدي الأخضر",
    state_code: "44",
  },
  {
    code: "44003",
    name: "عين البنيان",
    state_code: "44",
  },
  {
    code: "44004",
    name: "عين التركي",
    state_code: "44",
  },
  {
    code: "44005",
    name: "حمام ريغة",
    state_code: "44",
  },
  {
    code: "44006",
    name: "بوراشد",
    state_code: "44",
  },
  {
    code: "44007",
    name: "الحسينية",
    state_code: "44",
  },
  {
    code: "44008",
    name: "جليدة",
    state_code: "44",
  },
  {
    code: "44009",
    name: "عريب",
    state_code: "44",
  },
  {
    code: "44010",
    name: "جمعة أولاد الشيخ",
    state_code: "44",
  },
  {
    code: "44011",
    name: "العامرة",
    state_code: "44",
  },
  {
    code: "44012",
    name: "العطاف",
    state_code: "44",
  },
  {
    code: "44013",
    name: "تبركانين",
    state_code: "44",
  },
  {
    code: "44014",
    name: "عين بويحيى",
    state_code: "44",
  },
  {
    code: "44015",
    name: "العبادية",
    state_code: "44",
  },
  {
    code: "44016",
    name: "تاشتة زقاغة",
    state_code: "44",
  },
  {
    code: "44017",
    name: "بربوش",
    state_code: "44",
  },
  {
    code: "44018",
    name: "جندل",
    state_code: "44",
  },
  {
    code: "44019",
    name: "بن علال",
    state_code: "44",
  },
  {
    code: "44020",
    name: "وادي الشرفاء",
    state_code: "44",
  },
  {
    code: "44021",
    name: "بومدفع",
    state_code: "44",
  },
  {
    code: "44022",
    name: "عين الاشياخ",
    state_code: "44",
  },
  {
    code: "44023",
    name: "عين السلطان",
    state_code: "44",
  },
  {
    code: "44024",
    name: "واد الجمعة",
    state_code: "44",
  },
  {
    code: "44025",
    name: "الماين",
    state_code: "44",
  },
  {
    code: "44026",
    name: "الروينة",
    state_code: "44",
  },
  {
    code: "44027",
    name: "زدين",
    state_code: "44",
  },
  {
    code: "44028",
    name: "بئر ولد خليفة",
    state_code: "44",
  },
  {
    code: "44029",
    name: "برج الأمير خالد",
    state_code: "44",
  },
  {
    code: "44030",
    name: "طارق بن زياد",
    state_code: "44",
  },
  {
    code: "44031",
    name: "بطحية",
    state_code: "44",
  },
  {
    code: "44032",
    name: "بلعاص",
    state_code: "44",
  },
  {
    code: "44033",
    name: "الحسانية",
    state_code: "44",
  },
  {
    code: "44034",
    name: "عين الدفلى",
    state_code: "44",
  },
  {
    code: "44035",
    name: "مليانة",
    state_code: "44",
  },
  {
    code: "44036",
    name: "المخاطرية",
    state_code: "44",
  },
  {
    code: "45001",
    name: "تيوت",
    state_code: "45",
  },
  {
    code: "45002",
    name: "مغرار",
    state_code: "45",
  },
  {
    code: "45003",
    name: "عسلة",
    state_code: "45",
  },
  {
    code: "45004",
    name: "القصدير",
    state_code: "45",
  },
  {
    code: "45005",
    name: "مكمن بن عمار",
    state_code: "45",
  },
  {
    code: "45006",
    name: "عين الصفراء",
    state_code: "45",
  },
  {
    code: "45007",
    name: "المشرية",
    state_code: "45",
  },
  {
    code: "45008",
    name: "البيوض",
    state_code: "45",
  },
  {
    code: "45009",
    name: "عين بن خليل",
    state_code: "45",
  },
  {
    code: "45010",
    name: "النعامة",
    state_code: "45",
  },
  {
    code: "45011",
    name: "جنين بورزق",
    state_code: "45",
  },
  {
    code: "45012",
    name: "سفيسيفة",
    state_code: "45",
  },
  {
    code: "46001",
    name: "سيدي بومدين",
    state_code: "46",
  },
  {
    code: "46002",
    name: "تامزورة",
    state_code: "46",
  },
  {
    code: "46003",
    name: "شعبة اللحم",
    state_code: "46",
  },
  {
    code: "46004",
    name: "المالح",
    state_code: "46",
  },
  {
    code: "46005",
    name: "أولاد الكيحل",
    state_code: "46",
  },
  {
    code: "46006",
    name: "شنتوف",
    state_code: "46",
  },
  {
    code: "46007",
    name: "تارقة",
    state_code: "46",
  },
  {
    code: "46008",
    name: "وادي الصباح",
    state_code: "46",
  },
  {
    code: "46009",
    name: "العامرية",
    state_code: "46",
  },
  {
    code: "46010",
    name: "حاسي الغلة",
    state_code: "46",
  },
  {
    code: "46011",
    name: "أولاد بوجمعة",
    state_code: "46",
  },
  {
    code: "46012",
    name: "أغلال",
    state_code: "46",
  },
  {
    code: "46013",
    name: "عين الكيحل",
    state_code: "46",
  },
  {
    code: "46014",
    name: "عين الطلبة",
    state_code: "46",
  },
  {
    code: "46015",
    name: "عقب الليل",
    state_code: "46",
  },
  {
    code: "46016",
    name: "بني صاف",
    state_code: "46",
  },
  {
    code: "46017",
    name: "الحساسنة",
    state_code: "46",
  },
  {
    code: "46018",
    name: "الأمير عبد القادر",
    state_code: "46",
  },
  {
    code: "46019",
    name: "سيدي صافي",
    state_code: "46",
  },
  {
    code: "46020",
    name: "ولهاصة الغرابة",
    state_code: "46",
  },
  {
    code: "46021",
    name: "سيدي ورياش",
    state_code: "46",
  },
  {
    code: "46022",
    name: "عين الأربعاء",
    state_code: "46",
  },
  {
    code: "46023",
    name: "المساعيد",
    state_code: "46",
  },
  {
    code: "46024",
    name: "وادي برقش",
    state_code: "46",
  },
  {
    code: "46025",
    name: "سيدي بن عدة",
    state_code: "46",
  },
  {
    code: "46026",
    name: "عين تموشنت",
    state_code: "46",
  },
  {
    code: "46027",
    name: "بوزجار",
    state_code: "46",
  },
  {
    code: "46028",
    name: "حمام بوحجر",
    state_code: "46",
  },
  {
    code: "47001",
    name: "ضاية بن ضحوة",
    state_code: "47",
  },
  {
    code: "47002",
    name: "المنصورة",
    state_code: "47",
  },
  {
    code: "47003",
    name: "العطف",
    state_code: "47",
  },
  {
    code: "47004",
    name: "بونورة",
    state_code: "47",
  },
  {
    code: "47005",
    name: "زلفانة",
    state_code: "47",
  },
  {
    code: "47006",
    name: "القرارة",
    state_code: "47",
  },
  {
    code: "47007",
    name: "سبسب",
    state_code: "47",
  },
  {
    code: "47008",
    name: "متليلي",
    state_code: "47",
  },
  {
    code: "47009",
    name: "بريان",
    state_code: "47",
  },
  {
    code: "47010",
    name: "غرداية",
    state_code: "47",
  },
  {
    code: "48001",
    name: "القطار",
    state_code: "48",
  },
  {
    code: "48002",
    name: "أولاد يعيش",
    state_code: "48",
  },
  {
    code: "48003",
    name: "بني درقن",
    state_code: "48",
  },
  {
    code: "48004",
    name: "دار بن عبد الله",
    state_code: "48",
  },
  {
    code: "48005",
    name: "زمورة",
    state_code: "48",
  },
  {
    code: "48006",
    name: "جديوية",
    state_code: "48",
  },
  {
    code: "48007",
    name: "حمري",
    state_code: "48",
  },
  {
    code: "48008",
    name: "بلعسل بوزقزة",
    state_code: "48",
  },
  {
    code: "48009",
    name: "المطمر",
    state_code: "48",
  },
  {
    code: "48010",
    name: "سيدي  خطاب",
    state_code: "48",
  },
  {
    code: "48011",
    name: "سيدي امحمد بن عودة",
    state_code: "48",
  },
  {
    code: "48012",
    name: "عين طارق",
    state_code: "48",
  },
  {
    code: "48013",
    name: "حد الشكالة",
    state_code: "48",
  },
  {
    code: "48014",
    name: "الولجة",
    state_code: "48",
  },
  {
    code: "48015",
    name: "مازونة",
    state_code: "48",
  },
  {
    code: "48016",
    name: "عين الرحمة",
    state_code: "48",
  },
  {
    code: "48017",
    name: "القلعة",
    state_code: "48",
  },
  {
    code: "48018",
    name: "سيدي سعادة",
    state_code: "48",
  },
  {
    code: "48019",
    name: "يلل",
    state_code: "48",
  },
  {
    code: "48020",
    name: "سوق الحد",
    state_code: "48",
  },
  {
    code: "48021",
    name: "منداس",
    state_code: "48",
  },
  {
    code: "48022",
    name: "وادي السلام",
    state_code: "48",
  },
  {
    code: "48023",
    name: "سيدي لزرق",
    state_code: "48",
  },
  {
    code: "48024",
    name: "عمي موسى",
    state_code: "48",
  },
  {
    code: "48025",
    name: "واريزان",
    state_code: "48",
  },
  {
    code: "48026",
    name: "مرجة سيدي عابد",
    state_code: "48",
  },
  {
    code: "48027",
    name: "أولاد سيدي الميهوب",
    state_code: "48",
  },
  {
    code: "48028",
    name: "بن داود",
    state_code: "48",
  },
  {
    code: "48029",
    name: "وادي رهيو",
    state_code: "48",
  },
  {
    code: "48030",
    name: "الحاسي",
    state_code: "48",
  },
  {
    code: "48031",
    name: "سيدي أمحمد بن علي",
    state_code: "48",
  },
  {
    code: "48032",
    name: "مديونة",
    state_code: "48",
  },
  {
    code: "48033",
    name: "بني زنطيس",
    state_code: "48",
  },
  {
    code: "48034",
    name: "وادي الجمعة",
    state_code: "48",
  },
  {
    code: "48035",
    name: "لحلاف",
    state_code: "48",
  },
  {
    code: "48036",
    name: "غليزان",
    state_code: "48",
  },
  {
    code: "48037",
    name: "الحمادنة",
    state_code: "48",
  },
  {
    code: "48038",
    name: "الرمكة",
    state_code: "48",
  },
  {
    code: "49001",
    name: "تنركوك",
    state_code: "49",
  },
  {
    code: "49002",
    name: "تيميمون",
    state_code: "49",
  },
  {
    code: "49003",
    name: "أولاد السعيد",
    state_code: "49",
  },
  {
    code: "49004",
    name: "المطارفة",
    state_code: "49",
  },
  {
    code: "49005",
    name: "طالمين",
    state_code: "49",
  },
  {
    code: "49006",
    name: "أولاد عيسى",
    state_code: "49",
  },
  {
    code: "49007",
    name: "شروين",
    state_code: "49",
  },
  {
    code: "49008",
    name: "أوقروت",
    state_code: "49",
  },
  {
    code: "49009",
    name: "دلدول",
    state_code: "49",
  },
  {
    code: "49010",
    name: "قصر قدور",
    state_code: "49",
  },
  {
    code: "50001",
    name: "تيمياوين",
    state_code: "50",
  },
  {
    code: "50002",
    name: "برج باجي مختار",
    state_code: "50",
  },
  {
    code: "51001",
    name: "رأس الميعاد",
    state_code: "51",
  },
  {
    code: "51002",
    name: "بسباس",
    state_code: "51",
  },
  {
    code: "51003",
    name: "سيدي  خالد",
    state_code: "51",
  },
  {
    code: "51004",
    name: "الدوسن",
    state_code: "51",
  },
  {
    code: "51005",
    name: "الشعيبة",
    state_code: "51",
  },
  {
    code: "51006",
    name: "أولاد جلال",
    state_code: "51",
  },
  {
    code: "52001",
    name: "بني عباس",
    state_code: "52",
  },
  {
    code: "52002",
    name: "تامترت",
    state_code: "52",
  },
  {
    code: "52003",
    name: "إقلي",
    state_code: "52",
  },
  {
    code: "52004",
    name: "الواتة",
    state_code: "52",
  },
  {
    code: "52005",
    name: "أولاد خضير",
    state_code: "52",
  },
  {
    code: "52006",
    name: "كرزاز",
    state_code: "52",
  },
  {
    code: "52007",
    name: "تيمودي",
    state_code: "52",
  },
  {
    code: "52008",
    name: "القصابي",
    state_code: "52",
  },
  {
    code: "52009",
    name: "بن يخلف",
    state_code: "52",
  },
  {
    code: "53001",
    name: "إينغر",
    state_code: "53",
  },
  {
    code: "53002",
    name: "عين صالح",
    state_code: "53",
  },
  {
    code: "53003",
    name: "فقارة الزوى",
    state_code: "53",
  },
  {
    code: "54001",
    name: "تين زواتين",
    state_code: "54",
  },
  {
    code: "54002",
    name: "عين قزام",
    state_code: "54",
  },
  {
    code: "55001",
    name: "تماسين",
    state_code: "55",
  },
  {
    code: "55002",
    name: "سيدي سليمان",
    state_code: "55",
  },
  {
    code: "55003",
    name: "المقارين",
    state_code: "55",
  },
  {
    code: "55004",
    name: "النزلة",
    state_code: "55",
  },
  {
    code: "55005",
    name: "بلدة اعمر",
    state_code: "55",
  },
  {
    code: "55006",
    name: "تبسبست",
    state_code: "55",
  },
  {
    code: "55007",
    name: "تقرت",
    state_code: "55",
  },
  {
    code: "55008",
    name: "الطيبات",
    state_code: "55",
  },
  {
    code: "55009",
    name: "العالية",
    state_code: "55",
  },
  {
    code: "55010",
    name: "الحجيرة",
    state_code: "55",
  },
  {
    code: "55011",
    name: "بن ناصر",
    state_code: "55",
  },
  {
    code: "55012",
    name: "المنقر",
    state_code: "55",
  },
  {
    code: "55013",
    name: "الزاوية العابدية",
    state_code: "55",
  },
  {
    code: "56001",
    name: "جانت",
    state_code: "56",
  },
  {
    code: "56002",
    name: "برج الحواس",
    state_code: "56",
  },
  {
    code: "57001",
    name: "أم الطيور",
    state_code: "57",
  },
  {
    code: "57002",
    name: "سيدي عمران",
    state_code: "57",
  },
  {
    code: "57003",
    name: "المرارة",
    state_code: "57",
  },
  {
    code: "57004",
    name: "جامعة",
    state_code: "57",
  },
  {
    code: "57005",
    name: "تندلة",
    state_code: "57",
  },
  {
    code: "57006",
    name: "المغير",
    state_code: "57",
  },
  {
    code: "57007",
    name: "سطيل",
    state_code: "57",
  },
  {
    code: "57008",
    name: "سيدي خليل",
    state_code: "57",
  },
  {
    code: "58001",
    name: "المنيعة",
    state_code: "58",
  },
  {
    code: "58002",
    name: "حاسي القارة",
    state_code: "58",
  },
  {
    code: "58003",
    name: "حاسي الفحل",
    state_code: "58",
  },
];

type Props = Levelup.CMS.V1.Content.CustomFields.Forms.MetaFieldInputProps<
  "algerian_city",
  boolean
>;

const AlgerianCityCustomField: React.FC<Props> = ({
  label,
  required,
  value,
  onChange,
  options,
  default_value,
  metaData,
}) => {


  logger.value('metaData', metaData, options);

  /* -------------------------------------------------------------------------- */
  /*                                    RETURN                                  */
  /* -------------------------------------------------------------------------- */

  return <SelectCustomField {...{
    label,
    required,
    value: cities.filter(item => {
      return options.constraints ? options.constraints.reduce((previous, current) => previous || (!metaData[current.field] || metaData[current.field] === item.state_code), false) : true
    }).filter(({ code }) => code === value).map(({ code, name }) => value).reduce((previous, current) => options.multiple ? [...previous, current] : current, []) || null,
    onChange,
    options: {
      ...options,
      choices: cities.filter(item => {
        return options.constraints ? options.constraints.reduce((previous, current) => previous || (metaData[current.field] && metaData[current.field] === item.state_code), false) : true
      }).map(({ code, name }) => ({ value: code, label: name })),
    },
    default_value,
    metaData,
  }} />;
};

export default AlgerianCityCustomField;
