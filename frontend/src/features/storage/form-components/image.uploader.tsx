...
const ImageUploader: FC<Props> = ({ value, onUpload, onRemove, imageRatio = 1, containerClassname = "", dimensions, placeholder }) => {
  /* -------------------------------------------------------------------------- */
  /*                                    TOOLS                                   */
  /* -------------------------------------------------------------------------- */
  const sdk = useSdk();
  ...
  const [uploadedFile, setUploadedFile] = useState<Levelup.CMS.V1.Storage.Entity.UploadedFile | null>(null);
  ...
  return (
    <section className="text-center">
      <div {...getRootProps({ className: "dropzone" })} className="inline-block cursor-pointer">
        <input {...getInputProps()} />
        <aside className="mt-2 flex flex-wrap">
          <div className={clsx("relative inline-flex max-w-full border border-dashed border-gray-200 p-1", containerClassname)}>
            <div className="relative flex w-full min-w-0 overflow-hidden rounded-md">
              {placeholder && !uploadedFile && !selectedFile?.preview && !value?.id && !value?.url ? (
                placeholder
              ) : (
                <Image
                  src={
              ...
              {!selectedFile && (
                <span className={clsx("absolute cursor-pointer rounded-full bg-gray-900/70 p-2  text-white hover:bg-gray-900", "bottom-2 right-2")}>
                  <Icons.Edit className="h-5 w-5" />
                </span>
              )}
            </div>

            {isDragActive && !isDragReject ? <div className="absolute h-full w-full rounded-full border-2 border-blue-500 bg-blue-300/50"></div> : null}
            ...
            {selectedFile && !isDragActive && (
              <button
                ...
                className={clsx("absolute cursor-pointer text-red-300 hocus:text-red-500", " right-2 top-2  ")}
              >
            ...
            {processing && (
              <span className="absolute left-1/2 top-1/2 h-10 w-10 -translate-x-5 -translate-y-5 rounded-full bg-black/10 p-0">
                <svg className="h-10 w-10 -rotate-90 ">
                  <circle className="text-white/50" strokeWidth="3" stroke="currentColor" fill="transparent" r="17" cx="20" cy="20" />
                  <circle
                    className="text-green-500"
                    strokeWidth="3"
                    strokeDasharray={circumference}
                    strokeDashoffset={circumference - ((progressInfos.percentage || 0) / 100) * circumference}
                    strokeLinecap="round"
                    stroke="currentColor"
                    fill="transparent"
                    r="17"
