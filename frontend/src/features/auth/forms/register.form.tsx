...
const RegisterForm: React.FC<RegisterFormProps> = ({ }) => {
  ...
  /*                                    FORRM                                   */
  /* -------------------------------------------------------------------------- */
  const form = useForm<Levelup.CMS.V1.Utils.DeepComplete<ApiAlias.Request['data']> & { confirm_password: string; }, Validator<unknown, yup.AnySchema>>(
    {
      defaultValues: {
        account_type: 'agency',
        first_name: 'أحمد',
  ...
  return (
    <div className="">
      ...
      <div className="px-4 py-6 max-w-full ">
        ...
        <form.Field
          ...
          children={field => (
            <>
                    ...
                    >
                      <div className="value text-xl">
                        {!field.state.value?.state_code ? (
                          ...
                          <span>
                            {states?.find(index => index.code === field.state.value?.state_code)?.name || ""}
                          </span>
                        )}
                      </div>
                          ...
                          {states?.map(item => (
                            <CommandItem
                              ...
                              onSelect={value => {
                                field.handleChange((old) => ({
                                  ...old,
                                  state_code: value || '',
                                  state_name: states?.find(index => index.code === value)?.name || '',
                                  city_code: ''
                                }));
                                setWilayaOpen(false);
                              }}
                            ...
                            >
                              ...
                              <LuCheck
                                className={cn(
                                  "ms-auto",
                                  item.code === field.state.value.state_code ? "opacity-100" : "opacity-0",
                                )}
                              />
                            </CommandItem>
                          ))}
              ...
              {/* field */}
              <div className="field mb-4">
                <Label className="text-2xl text-darkblue-500">{"البلدية"}</Label>
                    ...
                    >
                      <div className="value text-xl">
                        {!field.state.value?.city_code ? (
                          ...
                          <span>
                            {cities?.find(index => index.code === field.state.value?.city_code)?.name || ""}
                          </span>
                        )}
                      </div>
                      <LuChevronsUpDown className="opacity-50" />
                  ...
                  >
                    ...
                    >
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {cities
                            ?.filter(item => item.state_code === field.state.value.state_code)
                            .map(item => (
                              <CommandItem
                                key={item.code}
                                value={item.code}
                                onSelect={value => {
                                  field.handleChange((old) => ({
                                    ...old,
                                    city_code: value || '',
                                    city_name: cities?.find(index => index.code === value)?.name || '',
                                  }));
                                  setCityOpen(false);
                                }}
                                className="text-xl"
                              >
                                ...
                                <LuCheck
                                  className={cn(
                                    "ms-auto",
                                    item.code === field.state.value.city_code ? "opacity-100" : "opacity-0",
                                  )}
                                />
                              </CommandItem>
                            ))}
              ...
              <div className="field mb-4">
                ...
                <Textarea
                  ...
                  onChange={(event) => field.handleChange((old) => ({
                    ...old,
        ...
        {/* field */}
        <div className="field mb-6">
          <Label className="text-2xl text-darkblue-500">{"تأكيد كلمة المرور"}</Label>
          <form.Field
